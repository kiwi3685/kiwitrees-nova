...
if (in_array($action, $actionPage)) {
    $actionA = $action;
} else {
    $actionB = $action;
}

// Cases where full 'edit_interface-page' is used.
switch ($actionA) {
    ////////////////////////////////////////////////////////////////////////////////
    case 'add':
    ...
    case 'addnewrepository':
        ...
        $controller->setPageTitle(KT_I18N::translate('Create a new Repository')); ?>
        ...
        echo pageStart('edit_interface', $controller->getPageTitle()); ?>
            ...
            <?php if ($rows) { ?>
				<div class="cell callout warning">
                    <?php echo KT_I18N::translate('
                        These individuals might be duplicates of your new entry.
                        Click on a name to open a new tab at their page to view
                        more details.
                        <br><br>
                        Close this window and the <strong>Add new individual
                        </strong> window if you do not want to complete this addition.
                    '); ?>
                </div>

				<h4><?php echo KT_I18N::translate('
                    Found %s possible duplicates', count($rows)); ?>
                </h4>
            ...
            <?php } else { ?>
                <div class="cell callout alert">
                    <?php echo KT_I18N::translate('No duplicates found'); ?>
                </div>
            <?php } ?>

            <div class="cell align-left">
      		   <button class="button primary" type="button" onclick="window.close();">
    ...
    case 'reorder_children':
        $controller
            ...
            echo pageStart('edit_interface', $controller->getPageTitle()); ?>
    			...
    			<div class="cell callout warning">
    				<?php echo KT_I18N::translate('Either click the "sort by date" button or select a row then drag-and-drop to re-order.'); ?>
    			</div>
    ...
    case 'reorder_media':
        ...
        echo pageStart('edit_interface', $controller->getPageTitle()); ?>
            ...
            <?php require_once KT_ROOT . 'includes/media_reorder.php'; ?>
...
//Cases where full 'edit_interface-page' is NOT used.
switch ($actionB) {
    ...
    case 'addchildaction':
        ...
        if (KT_Filter::postBool('SOUR_INDI')) {
    ...
    case 'addopfchildaction':
        ...
        if ($indirec) {
            $indirec .= "\n1 FAMS @{$newfamxref}@";
            $success = replace_gedrec($pid, KT_GED_ID, $indirec, $update_CHAN) && append_gedrec($gedrec, KT_GED_ID) && append_gedrec($famrec, KT_GED_ID);
        } else {
            $success = false;
        }
    ...
    case 'addnewparentaction':
        ...
        if ($famid == "new") {
            ...
            if (KT_Filter::postBool('SOUR_FAM')) {
    ...
    case 'addrepoaction':
        ...
        if (isset($_REQUEST['REPO_NAME'])) {
            $NAME = $_REQUEST['REPO_NAME'];
        }
        ...
        if (!empty($spid)) {
            ...
            if ($gedrec) {
                if ($famid == "new") {
                    ...
                    if (KT_Filter::postBool('SOUR_FAM') || count($tagSOUR) > 0) {
    ...
    case 'update':
        ...
        foreach ($cens_pids as $pid) {
            ...
            if (!is_array($linenum)) {
                ...
                if (isset($_REQUEST['NPFX'])) {
                    $NPFX = $_REQUEST['NPFX'];
                }
                if (isset($_REQUEST['GIVN'])) {
                    $GIVN = $_REQUEST['GIVN'];
                }
                if (isset($_REQUEST['NICK'])) {
                    $NICK = $_REQUEST['NICK'];
                }
                if (isset($_REQUEST['SPFX'])) {
                    $SPFX = $_REQUEST['SPFX'];
                }
                if (isset($_REQUEST['SURN'])) {
                    $SURN = $_REQUEST['SURN'];
                }
                if (isset($_REQUEST['NSFX'])) {
                    $NSFX = $_REQUEST['NSFX'];
                }
                if (isset($_REQUEST['ROMN'])) {
                    $ROMN = $_REQUEST['ROMN'];
                }
                if (isset($_REQUEST['FONE'])) {
                    $FONE = $_REQUEST['FONE'];
                }
                if (isset($_REQUEST['_HEB'])) {
                    $_HEB = $_REQUEST['_HEB'];
                }
                if (isset($_REQUEST['_AKA'])) {
                    $_AKA = $_REQUEST['_AKA'];
                }
                if (isset($_REQUEST['_MARNM'])) {
                    $_MARNM = $_REQUEST['_MARNM'];
                }
