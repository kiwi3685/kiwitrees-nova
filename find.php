...
$type		= KT_Filter::get('type', KT_REGEX_ALPHA, 'indi');
$filter		= KT_Filter::get('filter');
$action		= KT_Filter::get('action');
$callback	= KT_Filter::get('callback', KT_REGEX_NOSCRIPT, 'paste_id');
$media		= KT_Filter::get('media');
$all		= KT_Filter::getBool('all');
$subclick	= KT_Filter::get('subclick');
$choose		= KT_Filter::get('choose', KT_REGEX_NOSCRIPT, '0all');
$qs			= KT_Filter::get('tags');
...
<div id="find-page" class="grid-x">
	...
	// Show facts
	if ($type == "facts") { ?>
		<div id="find-facts-header">
			<form name="filterfacts" method="get" action="find.php">
				<input type="hidden" name="type" value="facts">
				<input type="hidden" name="tags" value="<?php echo $qs; ?>">
				<input type="hidden" name="callback" value="<?php echo $callback; ?>">
				<table class="list_table width100" border="0">
					<tr>
						<td class="list_label" style="padding: 5px; font-weight: normal; white-space: normal;">
							<?php getPreselectedTags($preselDefault, $preselCustom); ?>
							<script>
							...
							<div id="layDefinedTags">
								<table id="tabDefinedTags">
									<thead>
										<tr>
											<th>&nbsp;</th>
											<th><?php echo KT_I18N::translate('Tag'); ?></th>
							...
							<table id="tabDefinedTagsShow">
								<tbody>
									<tr>
										<td>
											<a href="#" onclick="Lister.showSelected();return false">
												<?php echo KT_I18N::translate('Show only selected tags'); ?>(<span id="layCurSelectedCount"></span>)
											</a>
										</td>
										<td>
											<a href="#" onclick="Lister.refreshNow(true);return false">
												<?php echo KT_I18N::translate('Show all tags'); ?>
											</a>
										</td>
									</tr>
								</tbody>
							</table>
							<table id="tabFilterAndCustom">
								<tbody>
									<tr>
										<td>
											<?php echo KT_I18N::translate('Filter'); ?>:
										</td>
										<td>
											<input type="text" id="tbxFilter">
										</td>
									</tr>
									<tr>
										<td>
											<?php echo KT_I18N::translate('Custom tags'); ?>:
										</td>
										<td>
											<input type="text" id="tbxCustom" value="<?php echo addslashes(implode(",", $preselCustom)); ?>">
										</td>
									</tr>
								</tbody>
							</table>
							<table id="tabAction">
								<tbody>
									<tr>
										<td colspan="2">
											<button id="btnOk" disabled="disabled" onclick="if (!this.disabled)DoOK();">
	...
	if ($action == "filter") {
		...
		if ($type == "specialchar") { ?>
			<div id="find-output-special" class="callout">
				<p>
					<!-- lower case special characters -->
					<?php foreach ($lcspecialchars as $key=>$value) { ?>
						<a class="largechars" href="#" onclick="return window.opener.paste_char('<?php echo $value; ?>');">
							<?php echo $key; ?>
						</a>
					<?php } ?>
				</p>
				<p>
					...
					<?php foreach ($ucspecialchars as $key=>$value) { ?>
						<a class="largechars" href="#" onclick="return window.opener.paste_char('<?php echo $value; ?>');">
							<?php echo $key; ?>
						</a>
					<?php } ?>
				...
				<p>
					...
					<?php foreach ($otherspecialchars as $key=>$value) { ?>
						<a class="largechars" href="#" onclick="return window.opener.paste_char('<?php echo $value; ?>');">
							<?php echo $key; ?>
						</a>
					<?php } ?>
