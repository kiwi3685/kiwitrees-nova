...
if ($action == 'update_mods' && KT_Filter::checkCsrf()) {
	foreach ($modules as $module_name => $module) {
		foreach (KT_Tree::getAll() as $tree) {
			...
			KT_DB::prepare(
				"REPLACE INTO `##module_privacy` (module_name, gedcom_id, component, access_level) VALUES (?, ?, 'tabf', ?)"
...
echo pageStart('famtabs', $controller->getPageTitle()); ?>
	...
	<form class="cell" method="post" action="<?php echo KT_SCRIPT_NAME; ?>">
			...
			<table id="tabf_table" class="modules_table">
				<thead>
					<tr>
											...
											<td>
												<?php
													$access_level = KT_DB::prepare(
														"SELECT access_level FROM `##module_privacy` WHERE gedcom_id=? AND module_name=? AND component='tabf'"
