...
switch (KT_Filter::post('action')) {
    case 'accept-changes':
        ...
        if ($record && KT_USER_CAN_ACCEPT && $record->canDisplayDetails() && $record->canEdit()) {
            KT_FlashMessages::addMessage(/* I18N: %s is the name of an individual, source or other record */ KT_I18N::translate('The changes to “%s” have been accepted.', $record->getFullName()), 'alert');
    ...
    case 'copy-fact':
        ...
        if (preg_match('/^(NOTE|SOUR|OBJE)$/', $fact->getTag())) {
            ...
            $type = KT_Filter::post('type', ['INDI', 'FAM', 'SOUR', 'REPO', 'OBJE', 'NOTE']);
        }
        if (!is_array($KT_SESSION->clipboard)) {
            $KT_SESSION->clipboard = [];
        }
        $KT_SESSION->clipboard[] = [
            'type' => $type,
        ...
        KT_FlashMessages::addMessage(KT_I18N::translate('Record copied to clipboard'), 'alert');
        ...
        if ($record && KT_USER_CAN_EDIT && $record->canDisplayDetails() && $record->canEdit()) {
            ...
            foreach (fetch_all_links($record->getXref(), $record->getGedId()) as $xref) {
                ...
                if (preg_match('/^0 @'.KT_REGEX_XREF.'@ FAM/', $gedrec) && preg_match_all('/\n1 (HUSB|WIFE|CHIL) @('.KT_REGEX_XREF.')@/', $gedrec, $match) < 2) {
                    ...
                    KT_FlashMessages::addMessage(/* I18N: %s is the name of a family group, e.g. “Husband name + Wife name” */ KT_I18N::translate('The family “%s” has been deleted, as it only has one member.', $family->getFullName()), 'alert');
                    ...
                    KT_FlashMessages::addMessage(/* I18N: %s are names of records, such as sources, repositories or individuals */ KT_I18N::translate('The link from “%1$s” to “%2$s” has been deleted.', $linker->getFullName(), $record->getFullName()), 'alert');
