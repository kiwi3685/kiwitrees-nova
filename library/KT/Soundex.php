...
class KT_Soundex {
	public static function getAlgorithms() {
		return array(
			'std' => /* I18N: http://en.wikipedia.org/wiki/Soundex */                 KT_I18N::translate('Russell'),
			'dm'  => /* I18N: http://en.wikipedia.org/wiki/Daitch–Mokotoff_Soundex */ KT_I18N::translate('Daitch-Mokotoff'),
		);
	}
	...
	public static function soundex($algorithm, $text) {
		...
		case 'dm':  return self::soundex_dm ($text);
		default:    trigger_error('Bad argument to KT_Soundex::soundex()', E_USER_ERROR);
		}
	}
	...
	private static function DMSoundex($name) {
		...
		while (count($partialResult) !=0  && $currPos <= $lastPos) {
			...
			while ($thisEntry != '') {
				if (isset(self::$dmsounds[$thisEntry])) break;
			...
			if ($state != 1) { // Not at beginning of input string
				if ($currPos <= $lastPos) {
					...
					while ($nextEntry != '') {
						if (isset(self::$dmsounds[$nextEntry])) break;
						$nextEntry = substr($nextEntry, 0, -1); // Not in table: try a shorter chunk
					}
				} else $nextEntry = '';
				if ($nextEntry != '' && self::$dmsounds[$nextEntry][0] != '0') $state = 2; // Next chunk is a vowel
				else $state = 3;
			}
