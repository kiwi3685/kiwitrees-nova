...
abstract class KT_Module {
	...
	// Get a list of all the active, authorised footer blocks
	static public function getActiveFooters($ged_id = KT_GED_ID, $access_level = KT_USER_ACCESS_LEVEL) {
		static $footers = null;
		if ($footers === null) {
			$footers = self::getActiveModulesByComponent('footer', $ged_id, $access_level);
		}
		return $footers;
	}
	...
	// Check module (a) provides a sidebar and (b) we have permission to see.
	...
	public static function isActiveReport($ged_id, $module, $access_level) {
		return array_key_exists($module, self::getActiveModulesByComponent('report', $ged_id, $access_level));
	}

	// Check module (a) provides a report and (b) we have permission to see.
	public static function isActiveMenu($ged_id, $module, $access_level) {
		return array_key_exists($module, self::getActiveModulesByComponent('menu', $ged_id, $access_level));
	}
	...
	static public function getInstalledModules($status) {
		...
		while (($file = readdir($dir)) !== false) {
			if (preg_match('/^[a-zA-Z0-9_]+$/', $file) && file_exists(KT_ROOT . KT_MODULES_DIR . $file . '/module.php')) {
				...
				if ($module instanceof KT_Module_Footer) {
					KT_DB::prepare(
						"INSERT IGNORE INTO `##module_privacy` (module_name, gedcom_id, component, access_level)".
						" SELECT ?, gedcom_id, 'footer', ?".
						" FROM `##gedcom`"
					)->execute(array($module->getName(), $module->defaultAccessLevel()));
				}
			}
	...
	static public function setDefaultAccess($ged_id) {
		foreach (self::getInstalledModules('disabled') as $module) {
			...
			if ($module instanceof KT_Module_Footer) {
				KT_DB::prepare(
					"INSERT IGNORE `##module_privacy` (module_name, gedcom_id, component, access_level) VALUES (?, ?, 'footer', ?)"
				)->execute(array($module->getName(), $ged_id, $module->defaultAccessLevel()));
			}
		}
	}
