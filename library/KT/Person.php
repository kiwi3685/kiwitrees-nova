...
class KT_Person extends KT_GedcomRecord {
    ...
    protected function createPrivateGedcomRecord($access_level) {
        ...
        // Don't privatize sex.
        if (preg_match('/\n1 SEX [MFU]/', (string) $this->_gedrec, $match)) {
            $rec .= $match[0];
        }
        return $rec;
    }
    ...
    */
    ...
    function getSex() {
        if (is_null($this->sex)) {
            if (preg_match('/\n1 SEX ([MF])/', (string) $this->_gedrec, $match)) {
                $this->sex = $match[1];
            } else {
                $this->sex = 'U';
            }
    ...
    */
    ...
    static function sexImage($sex, $size = 'small', $style = '', $title = '') {
        ...
        case 'F':
            ...
            break;
        case 'U':
            $icon_sex = 'fa-genderless';
            break;
        }
        return '<i class="' . $iconStyle . ' ' . $icon_sex . $icon_size . '" title="' . $title . '"></i>';
    }
        ...
        if (is_object($elderdate) && $elderdate->isOK()) {
            ...
            if ($p2->isOK()) {
                ...
                // warning if negative gap : wrong order
                    if ($gap < 0 && $counter > 0){
                        $label .= '<i class="' . $iconStyle . ' fa-exclamation-triangle warning"></i> ';
                    }
    ...
    */
    ...
    function getNumberOfChildren() {
        if (preg_match('/\n1 NCHI (\d+)(?:\n|$)/', (string) $this->getGedcomRecord(), $match)) {
            return $match[1];
        } else {
            $children = array();
    ...
    */
    ...
    function getChildFamilyLabel(KT_Family $family) {
        if (preg_match('/\n1 FAMC @' . $family->getXref() . '@(?:\n[2-9].*)*\n2 PEDI (.+)/', (string) $this->getGedcomRecord(), $match)) {
    ...
    */
    function add_spouse_facts($spouse, $famrec = '') {
        ...
        if (preg_match('/\n1 (?:' . KT_EVENTS_DIV . ')\b/', (string) $famrec)) {
            return;
        }
        // Only include events between birth and death
        $bDate = $this->getEstimatedBirthDate();
            ...
            foreach ($associate->getFacts() as $event) {
                ...
                foreach (array('ASSO', '_ASSO') as $asso_tag) {
                    ...
                    if ($arec) {
                        ...
                        if (preg_match('/\n2 DATE .*/', (string) $srec, $match)) {
                            $factrec .= $match[0];
                        }
                        if (preg_match('/\n2 PLAC .*/', (string) $srec, $match)) {
                            $factrec .= $match[0];
                        }
    ...
    */
    ...
    protected function _addName($type, $full, $gedrec) {
        ...
        if (preg_match('/\/.*\//', (string) $full, $match)) {
        ...
        if ($NICK) {
            ...
            if (preg_match('/(^| |"|«|“|\'|‹|‘|„)' . preg_quote($NICK, '/') . '( |"|»|”|\'|›|’|”|$)/', (string) $full)) {
