...
class KT_GedcomRecord {
	...
	private   $disp_public = null;  // Can we display details of this record to KT_PRIV_PUBLIC
	private   $disp_user   = null;  // Can we display details of this record to KT_PRIV_USER
	private   $disp_none   = null;  // Can we display details of this record to KT_PRIV_NONE
	...
	static public function getInstance($data) {
		...
		if (!is_array($data)) {
			...
			if (KT_USER_CAN_EDIT) {
				...
				if (!isset($pending_record_cache[$ged_id]) || KT_SCRIPT_NAME=='edit_interface.php') {
		...
		switch($type) {
		case 'INDI':
			$object = new KT_Person($data);
			break;
		case 'FAM':
			$object = new KT_Family($data);
			break;
		case 'SOUR':
			$object = new KT_Source($data);
			break;
		case 'OBJE':
			$object = new KT_Media($data);
			break;
		case 'REPO':
			$object = new KT_Repository($data);
			break;
		case 'NOTE':
			$object = new KT_Note($data);
			break;
		default:
			$object = new KT_GedcomRecord($data);
			break;
		}
	...
	private static function fetchGedcomRecord($xref, $ged_id) {
		...
		$row = KT_Person::fetchGedcomRecord($xref, $ged_id);
		if ($row) {
			return $row;
		}
		$row = KT_Family::fetchGedcomRecord($xref, $ged_id);
		if ($row) {
			return $row;
		}
		$row = KT_Source::fetchGedcomRecord($xref, $ged_id);
		if ($row) {
			return $row;
		}
		$row = KT_Repository::fetchGedcomRecord($xref, $ged_id);
		if ($row) {
			return $row;
		}
		$row = KT_Media::fetchGedcomRecord($xref, $ged_id);
		if ($row) {
			return $row;
		}
		$row = KT_Note::fetchGedcomRecord($xref, $ged_id);
		if ($row) {
			return $row;
		}
	...
	*/
	public function getGedcomRecord() {
		if ($this->gedrec === null) {
			list($this->gedrec) = $this->privatizeGedcom(KT_USER_ACCESS_LEVEL);
		}
		return $this->gedrec;
	}
	/**
	...
	*/
	...
	protected function _getLinkUrl($link, $separator) {
		if ($this->ged_id) {
			...
			return $link.$this->getXref().$separator.'ged='.KT_GEDURL;
		}
	}
	...
	*/
	public function getAbsoluteLinkUrl() {
		return KT_SERVER_NAME.KT_SCRIPT_PATH.$this->getHtmlUrl();
	}
	...
	*/
	...
	private function _canDisplayDetails($access_level) {
		...
		if ($this->getXref() == KT_USER_GEDCOM_ID && $this->getGedId() == KT_GED_ID && $access_level == KT_USER_ACCESS_LEVEL) {
			return true;
		}
	...
	public function canDisplayDetails($access_level = KT_USER_ACCESS_LEVEL) {
	...
	protected function createPrivateGedcomRecord($access_level) {
		return "0 @".$this->xref."@ ".$this->type."\n1 NOTE ".KT_I18N::translate('Private');
	}
	...
	protected function _getAllNames($fact = '!', $level = 1) {
		...
		if (is_null($this->_getAllNames)) {
			...
			if ($this->canDisplayName()) {
				...
				$this->_addName($this->getType(), KT_I18N::translate('Private'), null);
			}
		}
	...
	public function getPrimaryName() {
		...
		if ($language_script === null) {
			$language_script = KT_I18N::languageScript(KT_LOCALE);
		}
	...
	static function CompareChanDate($x, $y) {
		...
		$tmp = KT_Date::Compare($chan_x->getDate(), $chan_y->getDate());
		if ($tmp) {
			return $tmp;
		} else {
			if (
	...
	public function format_list($tag = 'li', $find = false, $name = null) {
		...
		$html = '<a href="' . $this->getHtmlUrl() . '" ';
			$find ? $html .= ' onclick="pasteid(\''.$this->getXref().'\', \'' . htmlentities($name) . '\');" ' : $html .= '';
		$html .= 'class="list_item">' . $name . '</a><small>' . $this->format_list_details() . '</small>';
	...
	// Get the first KT_Event for the given fact type
	...
	*/
	...
	public function LastChangeUser() {
		...
		$chan_user = $chan->getValue('_KT_USER');
		if (empty($chan_user)) {
			return '&nbsp;';
		}
		return $chan_user;
