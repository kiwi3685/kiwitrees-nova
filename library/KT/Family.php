...
class KT_Family extends KT_GedcomRecord {
	private $husb = null;
	private $wife = null;
	...
	function __construct($data) {
		if (is_array($data)) {
			...
			if (preg_match('/^1 HUSB @(.+)@/m', $data['gedrec'], $match)) {
				$this->husb=KT_Person::getInstance($match[1]);
			}
			if (preg_match('/^1 WIFE @(.+)@/m', $data['gedrec'], $match)) {
				$this->wife=KT_Person::getInstance($match[1]);
			}
		} else {
	...
	protected function createPrivateGedcomRecord($access_level) {
		...
		preg_match_all('/\n1 (?:CHIL|HUSB|WIFE) @('.KT_REGEX_XREF.')@/', $this->_gedrec, $matches, PREG_SET_ORDER);
		foreach ($matches as $match) {
			$rela=KT_Person::getInstance($match[1]);
	...
	protected function _canDisplayDetailsByType($access_level) {
		...
		preg_match_all('/\n1 (?:CHIL|HUSB|WIFE) @('.KT_REGEX_XREF.')@/', $this->_gedrec, $matches);
		foreach ($matches[1] as $match) {
			$person=KT_Person::getInstance($match);
	 ...
	 * @return Person
	 */
	function getSpouse($person, $access_level=KT_USER_ACCESS_LEVEL) {
	...
	function getSpouses($access_level=KT_USER_ACCESS_LEVEL) {
		$spouses=array();
	 ...
	 */
	function getChildren($access_level=KT_USER_ACCESS_LEVEL) {
		global $SHOW_PRIVATE_RELATIONSHIPS;

		$children=array();
		preg_match_all('/\n1 CHIL @('.KT_REGEX_XREF.')@/', $this->_gedrec, $match);
		foreach ($match[1] as $pid) {
			$child=KT_Person::getInstance($pid);
	...
	static function CompareMarrDate($x, $y) {
		return KT_Date::Compare($x->getMarriageDate(), $y->getMarriageDate());
	}
	...
	function getUpdatedFamily() {
		...
		if (KT_USER_CAN_EDIT && $this->canDisplayDetails()) {
			...
			if (!is_null($newrec)) {
				$newfamily = new KT_Family($newrec);
	...
	function _parseMarriageRecord() {
		$this->marriage = new KT_Event(get_sub_record(1, '1 MARR', $this->getGedcomRecord()), $this, 0);
	}
	...
	function getAllMarriageDates() {
		if ($this->canDisplayDetails()) {
			foreach (explode('|', KT_EVENTS_MARR) as $event) {
	...
	public function getAllNames() {
		if (is_null($this->_getAllNames)) {
			$husb=$this->husb ? $this->husb : new KT_Person('1 SEX M');
			$wife=$this->wife ? $this->wife : new KT_Person('1 SEX F');
