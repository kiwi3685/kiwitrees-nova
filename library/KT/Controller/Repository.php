...
class KT_Controller_Repository extends KT_Controller_GedcomRecord {
	public function __construct() {
		...
		$this->record = new KT_Repository($gedrec);

		// If there are pending changes, merge them in.
		if ($newrec !== null) {
			$diff_record = new KT_Repository($newrec);
	...
	*/
	function getEditMenu() {
		...
		// edit menu
		$menu = new KT_Menu('<span>' . KT_I18N::translate('Edit') . '</span>', '#', 'menu-repo');
		$menu->addClass('', '', 'fa-pen-to-square');

		if (KT_USER_CAN_EDIT) {
			$submenu = new KT_Menu(KT_I18N::translate('Edit repository'), '#', 'menu-repo-edit');
		...
		if (KT_USER_IS_ADMIN || $SHOW_GEDCOM_RECORD) {
			$submenu = new KT_Menu(KT_I18N::translate('Edit raw GEDCOM record'), '#', 'menu-repo-editraw');
			$submenu->addOnclick("return edit_raw('".$this->record->getXref()."');");
			$menu->addSubmenu($submenu);
		} elseif ($SHOW_GEDCOM_RECORD) {
			$submenu = new KT_Menu(KT_I18N::translate('View GEDCOM Record'), '#', 'menu-repo-viewraw');
			if (KT_USER_CAN_EDIT) {
		...
		// delete
		if (KT_USER_CAN_EDIT) {
			$submenu = new KT_Menu(KT_I18N::translate('Delete'), '#', 'menu-repo-del');
			$submenu->addOnclick("if (confirm('".KT_I18N::translate('Are you sure you want to delete “%s”?', strip_tags($this->record->getFullName()))."')) jQuery.post('action.php',{action:'delete-repository',xref:'".$this->record->getXref()."'},function(){location.reload();})");
			$menu->addSubmenu($submenu);
		}
