...
class KT_Controller_Ancestry extends KT_Controller_Chart {
	...
	public function __construct() {
		...
		$this->generations	= KT_Filter::getInteger('PEDIGREE_GENERATIONS', 2, $MAX_PEDIGREE_GENERATIONS, $DEFAULT_PEDIGREE_GENERATIONS);
	...
	function print_child_ascendancy($person, $sosa, $depth) {
		...
		<li>
			<table>
				<tr>
					<td>
						<a name="sosa<?php echo $sosa; ?>"></a>
						<?php if ($sosa == 1) { ?>
							<img src="<?php echo $KT_IMAGES['spacer']; ?>" height="3" width="<?php echo $Dindent; ?>" alt="">
						<?php } else { ?>
							<img src="<?php echo $KT_IMAGES['spacer']; ?>" height="3" width="2" alt="">
							<img src="<?php echo  $KT_IMAGES['hline']; ?>" height="3" width="<?php echo $Dindent - 2; ?>" alt="">
						<?php }
				...
				<td>
					<?php if ($sosa > 1) {
						print_url_arrow($pid, '?rootid=' . $pid . '&amp;PEDIGREE_GENERATIONS=' . $this->generations . '&amp;show_full=' . $this->show_full . '&amp;chart_style=' . $this->chart_style . '&amp;ged=' . KT_GEDURL, $label, 3);
					} ?>
				</td>
				<td>
					<span class="<?php echo 'person_box_template ' . (($sosa==1) ? 'U' : (($sosa%2) ? 'F' : '')); ?>">&nbsp;<?php echo $sosa; ?>&nbsp;</span>&nbsp;
				</td>
				<td>
					<?php
					$relation	= '';
