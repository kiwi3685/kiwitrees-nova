...
class KT_Controller_Hourglass extends KT_Controller_Chart {
	...
	function __construct($rootid='', $show_full=1, $generations=3) {
		...
		if ($TEXT_DIRECTION=='ltr') {
			$this->left_arrow='fa fa-angle-left fa-2x';
			$this->right_arrow='fa fa-angle-right fa-2x';
		} else {
			$this->left_arrow='fa fa-angle-right fa-2x';
			$this->right_arrow='fa fa-angle-left fa-2x';
		}
		...
		// Validate parameters
		$this->hourPerson = KT_Person::getInstance($this->pid);
		if (!$this->hourPerson) {
		...
		$this->setPageTitle(/* I18N: %s is an individual’s name */ KT_I18N::translate('Hourglass chart of %s', $this->name));
	}
	...
	function print_person_pedigree($person, $count) {
		global $KT_IMAGES, $bheight, $bwidth, $bhalfheight;
		...
		foreach ($person->getChildFamilies() as $family) {
			echo "<table style=\"empty-cells: show;\">";
			$height="100%";
			echo "<tr>";
			echo "<td valign=\"bottom\"><img class=\"line3\" name=\"pvline\" src=\"".$KT_IMAGES["vline"]."\" width=\"3\" height=\"$lh\" alt=\"\"></td>";
			echo "<td><img class=\"line4\" src=\"".$KT_IMAGES["hline"]."\" width=\"7\" height=\"3\" alt=\"\"></td>";
			echo "<td>";
			...
			echo '</tr><tr>',
				 '<td valign="top"><img name="pvline" src="'.$KT_IMAGES["vline"].'" width="3" height="$lh" alt=""></td>',
				 '<td><img class="line4" src="'.$KT_IMAGES["hline"].'" width="7" height="3" alt=""></td>',
				 '<td>';
	...
	function print_descendency($person, $count, $showNav=true) {
		global $TEXT_DIRECTION, $KT_IMAGES, $bheight, $bwidth, $bhalfheight, $lastGenSecondFam;
		...
		echo "<table id=\"table_$pid\" align=\"".$tablealign."\">";
		echo "<tr>";
		...
		if ($count < $this->dgenerations) {
			...
			$ct = count($children);
			if ($ct>0) {
				echo "<table style=\"position: relative; top: auto; text-align: $tablealign;\">";
				for ($i=0; $i<$ct; $i++) {
					...
					if ($ct>1) {
						if ($i==0) {
							...
							echo "<td valign=\"bottom\"><img class=\"line1\" name=\"tvertline\" id=\"vline_$chil\" src=\"".$KT_IMAGES["vline"]."\" width=\"3\" height=\"$h\" alt=\"\"></td>";
						} else if ($i==$ct-1) {
							...
							echo "<td valign=\"top\"><img name=\"bvertline\" id=\"vline_$chil\" src=\"".$KT_IMAGES["vline"]."\" width=\"3\" height=\"".$h."\" alt=\"\"></td>";
						} else {
							echo "<td style=\"background: url('".$KT_IMAGES["vline"]."');\"><img src=\"".$KT_IMAGES["spacer"]."\" width=\"3\" alt=\"\"></td>";
						}
					}
					echo "</tr>";
		...
		echo "</td><td><img class=\"line2\" src=\"".$KT_IMAGES["hline"]."\" width=\"7\" height=\"3\" alt=\"\">";
				...
				if ($num>0) {
					...
					foreach ($cfamids as $family) {
						if ($family->getHusband() || $family->getWife()) {
							echo "<span class=\"name1\"><br>".KT_I18N::translate('Parents')."<br></span>";
						...
						$children = $family->getChildren();
						$num = $family->getNumberOfChildren();
						if ($num>2) echo "<span class=\"name1\"><br>".KT_I18N::translate('Siblings')."<br></span>";
						if ($num==2) echo "<span class=\"name1\"><br>".KT_I18N::translate('Sibling')."<br></span>";
						foreach ($children as $id=>$child) {
							$cid = $child->getXref();
							if ($cid!=$pid) {
								echo "&nbsp;&nbsp;<a href=\"hourglass.php?rootid={$cid}&amp;show_spouse={$this->show_spouse}&amp;show_full={$this->show_full}&amp;generations={$this->generations}&amp;box_width={$this->box_width}\" class=\"name1\">";
