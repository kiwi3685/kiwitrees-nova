...
class KT_Controller_Familybook extends KT_Controller_Chart {
	// Data for the view
	public $pid	       =null;
	...
	public function __construct() {
		...
		$PEDIGREE_FULL_DETAILS      =get_gedcom_setting(KT_GED_ID, 'PEDIGREE_FULL_DETAILS');
		$MAX_DESCENDANCY_GENERATIONS=get_gedcom_setting(KT_GED_ID, 'MAX_DESCENDANCY_GENERATIONS');

		// Extract the request parameters
		$this->show_full   = KT_Filter::getInteger('show_full',   0, 1, $PEDIGREE_FULL_DETAILS);
		$this->show_spouse = KT_Filter::getInteger('show_spouse', 0, 1);
		$this->descent     = KT_Filter::getInteger('descent',     0, 9, 5);
		$this->generations = KT_Filter::getInteger('generations', 2, $MAX_DESCENDANCY_GENERATIONS, 2);
		$this->box_width   = KT_Filter::getInteger('box_width',   50, 300, 100);
		...
		if ($this->root && $this->root->canDispLayname()) {
			$this->setPageTitle(
				...
				KT_I18N::translate('Family book of %s', $this->root->getFullName())
			);
		} else {
			$this->setPageTitle(KT_I18N::translate('Family book'));
		}
	...
	function print_descendency($person, $count) {
		global $TEXT_DIRECTION, $KT_IMAGES, $bwidth, $bheight, $bhalfheight;
			...
			if ($ct==0 ) { // empty boxes
				...
				for ($i=0; $i<1; $i++) { //set to 2 for two child boxes
					...
					if ($i==0) {
						...
						echo '<td style="background: url(',$KT_IMAGES["vline"],');">',
							 '<img class="spacer" src="',$KT_IMAGES["spacer"],'" alt=""></td>';
					}
					echo '</tr>';
				}
				echo '</table>';
				...
				for ($i=0; $i<$ct; $i++) {
					...
					if ($ct>1) {
						if ($i==0) {
							...
							if ($kids>1) $h = ((($kids-1)*4)+$h);
							echo '<td class="tdbot">',
								 '<img class="tvertline" id="vline_',$chil,'" src="',$KT_IMAGES["vline"],'"  height="',$h-1,'" alt=""></td>';
						} else if ($i==$ct-1) {
							//-- adjust for the first column on left
							...
							if ($kids>1) $h = ((($kids-1)*4)+$h);
							echo '<td class="tdtop">',
								 '<img class="bvertline" id="vline_',$chil,'" src="',$KT_IMAGES["vline"],'" height="',$h+1,'" alt=""></td>';
						} else {
							echo '<td style="background: url(',$KT_IMAGES["vline"],');">',
								 '<img class="spacer" src="',$KT_IMAGES["spacer"],'" alt=""></td>';
						}
		...
		if ($person) {
			print_pedigree_person($person);
					echo '</td><td>',
			 '<img class="line2" src="',$KT_IMAGES["hline"],'" width="8" height="3" alt="">';
		} else  {
	...
	function print_person_pedigree($person, $count) {
		...
		foreach ($person->getChildFamilies() as $family) {
			...
			echo '<img class="line3 pvline"  src="',$KT_IMAGES["vline"],'" height="',$lh-1,'" alt=""></td>',
				 '<td>',
				 '<img class="line4" src="',$KT_IMAGES["hline"],'" height="3" alt=""></td>',
				 '<td>';
			...
			echo '</tr><tr>',
				 '<td class="tdtop"><img class="pvline" src="',$KT_IMAGES["vline"],'" height="',$lh+1,'" alt=""></td>',
				 '<td><img class="line4" src="',$KT_IMAGES["hline"],'" height="3" alt=""></td>',
				 '<td>';
	...
	function print_family_book($person, $descent) {
		...
		if (count($families)>0 || empty($firstrun)) {
			...
			echo
				'<h3>',
					/* I18N: A title/heading. %s is an individual’s name */ KT_I18N::translate('Family of %s', $person->getFullName()),
				'</h3>
