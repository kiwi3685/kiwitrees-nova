...
class KT_Controller_Pedigree extends KT_Controller_Chart {
	var $rootid;
	var $name;
	var $addname;
	var $show_full;
	...
	public function __construct() {
		...
		if ($this->root && $this->root->canDisplayName()) {
			$this->setPageTitle(
				...
				KT_I18N::translate('Pedigree tree of %s', $this->root->getFullName())
			);
		} else {
			$this->setPageTitle(KT_I18N::translate('Pedigree'));
		}
	...
	function getPersonName() {
		if (is_null($this->root)) {
			return KT_I18N::translate('unknown');
		} else {
