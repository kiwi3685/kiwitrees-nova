...
class KT_Controller_Search extends KT_Controller_Page {
	public $action;
	...
	function __construct() {
		...
		if (!$this->isPostBack) {
			...
			$str = str_replace(array (".", "-", " "), array ("_", "_", "_"), KT_GEDCOM);
			$_REQUEST["$str"] = $str;
		}
		...
		if (count(KT_Tree::getAll())>1 && KT_Site::preference('ALLOW_CHANGE_GEDCOM')) {
			...
			$this->sgeds[KT_GED_ID] = KT_GEDCOM;
		}

		// vars use for soundex search
		$this->firstname = KT_Filter::post('firstname');
		$this->lastname  = KT_Filter::post('lastname');
		$this->place     = KT_Filter::post('place');
		$this->year      = KT_Filter::post('year');
	...
	function setRequestValues($varNames) {
		foreach ($varNames as $varName) {
			if (isset ($_REQUEST[$varName])) {
				if ($varName == 'action' && $_REQUEST[$varName] == 'replace' && !KT_USER_CAN_EDIT) {
					$this->action='general';
					continue;
				}
	...
	function getOtherFields() {
		...
		foreach ($ofields as $field) {
			$fields[$field] = KT_Gedcom_Tag::GetLabel($field);
		}
		uksort($fields, array('KT_Controller_Search', 'tagSort'));
		return $fields;
	}
	...
	public static function tagSort($x, $y) {
		...
		if ($tmp) {
			...
			return utf8_strcasecmp(KT_Gedcom_Tag::getLabel($x), KT_Gedcom_Tag::getLabel($y));
		}
	}
	...
	function getLabel($tag) {
		return KT_Gedcom_Tag::getLabel(preg_replace('/:(SDX|BEGINS|EXACT|CONTAINS)$/', '', $tag));
	...
	function TopSearch() {
		...
		if (preg_match('/' . KT_REGEX_XREF . '/', $this->query)) {
			...
			if ($record && $record->canDisplayDetails()) {
				...
				header('Location: '. KT_SERVER_NAME . KT_SCRIPT_PATH . $record->getRawUrl());
				exit;
			}
		}
	}
	...
	function GeneralSearch() {
		...
		if ($query_terms && $this->sgeds) {
			...
			// Search the stories
			if (isset ($this->srstor) && array_key_exists('stories', KT_Module::getActiveModules())) {
				if (!empty ($this->query))
	...
	function SearchAndReplace() {
		...
		$this->sgeds = array(KT_GED_ID=>KT_GEDCOM);
		$this->srindi = "yes";
		$this->srfams = "yes";
		$this->srsour = "yes";
		$this->srnote = "yes";
		...
		foreach ($this->myindilist as $id=>$individual) {
			$indirec = find_gedcom_record($individual->getXref(), KT_GED_ID, true);
			$oldRecord = $indirec;
			$newRecord = $indirec;
			if ($this->replaceAll) {
			...
			if ($newRecord != $oldRecord) {
				replace_gedrec($individual->getXref(), KT_GED_ID, $newRecord);
			} else {
		...
		foreach ($this->myfamlist as $id=>$family) {
			$indirec = find_gedcom_record($family->getXref(), KT_GED_ID, true);
			$oldRecord = $indirec;
			$newRecord = $indirec;
			...
			//-- if the record changed replace the record otherwise remove it from the search results
			if ($newRecord != $oldRecord) {
				replace_gedrec($family->getXref(), KT_GED_ID, $newRecord);
			} else {
		...
		foreach ($this->mysourcelist as $id=>$source) {
			$indirec = find_gedcom_record($source->getXref(), KT_GED_ID, true);
			$oldRecord = $indirec;
			$newRecord = $indirec;
			...
			//-- if the record changed replace the record otherwise remove it from the search results
			if ($newRecord != $oldRecord) {
				replace_gedrec($source->getXref(), KT_GED_ID, $newRecord);
			} else {
		...
		foreach ($this->mynotelist as $id=>$note) {
			$indirec = find_gedcom_record($note->getXref(), KT_GED_ID, true);
			$oldRecord = $indirec;
			$newRecord = $indirec;
			...
			//-- if the record changed replace the record otherwise remove it from the search results
			if ($newRecord != $oldRecord) {
				replace_gedrec($note->getXref(), KT_GED_ID, $newRecord);
			} else {
	...
	function AdvancedSearch ($table = "individuals") {
		...
		$rows = KT_DB::prepare($sql)->execute($bind)->fetchAll(PDO::FETCH_ASSOC);
		foreach ($rows as $row) {
	...
	function printResults() {
		...
		if ($this->action == "general" || $this->action == "soundex" || $this->action == "replace" || $this->action == "advanced") {
			if ($this->myindilist || $this->myfamlist || $this->mysourcelist || $this->mynotelist || $this->mystorieslist) {
				...
				if ($this->myindilist) {echo '<li><a href="#searchAccordion-indi"><span id="indisource">', KT_I18N::translate('Individuals'), '</span></a></li>';}
				if ($this->myfamlist) {echo '<li><a href="#searchAccordion-fam"><span id="famsource">', KT_I18N::translate('Families'), '</span></a></li>';}
				if ($this->mysourcelist) {echo '<li><a href="#searchAccordion-source"><span id="mediasource">', KT_I18N::translate('Sources'), '</span></a></li>';}
				if ($this->mynotelist) {echo '<li><a href="#searchAccordion-note"><span id="notesource">', KT_I18N::translate('Notes'), '</span></a></li>';}
				if ($this->mystorieslist) {echo '<li><a href="#searchAccordion-story"><span id="storysource">', KT_I18N::translate('Stories'), '</span></a></li>';}
				echo '</ul>';

				// individual results
				echo '<div id="searchAccordion-indi">';
					if ($this->action == "advanced") {
						uasort($this->myindilist, array('KT_GedcomRecord', 'Compare'));
						...
						foreach ($this->sgeds as $ged_id=>$gedcom) {
							...
							if ($datalist) {
								...
								usort($datalist, array('KT_GedcomRecord', 'Compare'));
								$GEDCOM = $gedcom;
