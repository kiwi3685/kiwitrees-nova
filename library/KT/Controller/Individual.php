...
class KT_Controller_Individual extends KT_Controller_GedcomRecord {
	...
	*/
	function print_name_record(KT_Event $event) {
		...
		<div class="accordion-item<?php echo $active . $old; ?>" data-accordion-item>
			...
			<div class="accordion-content" data-tab-content>
				<div class="grid-x grid-padding-x">
					<div class="cell medium-9">
						<div class="grid-x">
							...
							for ($i = 0; $i < $ct; $i ++) { ?>
								...
								<?php } ?>
							<?php }
							if ($this->name_count == 1 && KT_USER_IS_ADMIN) {
					...
					<?php if (preg_match("/\d (NOTE)|(SOUR)/", $factrec) > 0) { ?>
						<div id="indi_note" class="cell">
							<?php // -- find sources for this name
		...
		echo '<span id="sex"';
			...
			if ($line != 'new'){
				...
				case 'M':
					...
					echo '<i class="' . $iconStyle . ' fa-male"></i>';
					break;
				case 'F':
					if ($this->record->canEdit() && !$event->getIsOld()) {
					...
					echo '<i class="' . $iconStyle . ' fa-female"></i>';
					break;
				case 'U':
					if ($this->record->canEdit() && !$event->getIsOld()) {
						...
						echo '<i class="' . $iconStyle . ' fa-genderless"></i>';
					break;
				}
			} else {
	...
	* @param Person $person
	* @return string returns 'person_box', 'person_boxF', or 'person_boxU'
	*/
			...
			if ($module->hasSidebarContent()) {
				...
				$html .= '
					<li class="accordion-item' . $active . '" data-accordion-item>
						<a class="accordion-title" href="#">' . $module->getTitle() . ' </a>
						<div class="accordion-content" id="sb_content_' . $module->getName() . '" data-tab-content>' .
							$configLink .
							$module->getSidebarContent() . '
						</div>
					</li>
				';
			}
