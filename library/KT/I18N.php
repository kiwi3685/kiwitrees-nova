...
/**
 *  Class to support internationalisation (KT_I18N) functionality.
...
class KT_I18N {
	...
	public static function init($locale = null) {
		...
		if (is_null($locale) || !array_key_exists($locale, $installed_languages) || !array_key_exists($locale, $used_languages)) {
			...
			if (isset($_GET['lang']) && array_key_exists($_GET['lang'], $installed_languages) && array_key_exists($_GET['lang'], $used_languages)) {
				...
				unset($_GET['lang']);
				if (KT_USER_ID) {
					set_user_setting(KT_USER_ID, 'language', $locale);
				}
			} elseif (array_key_exists($KT_SESSION->locale, $installed_languages) && array_key_exists($KT_SESSION->locale, $used_languages)) {
				// Remembered from a previous visit?
				$locale = $KT_SESSION->locale;
			} else {
				...
				if (KT_GED_ID) {
					...
					$locale = get_gedcom_setting(KT_GED_ID, 'LANGUAGE');
					$prefs[] = $locale . ';q=0.2';
				}
				$prefs2 = array();
		...
		self::$translation_adapter = new Zend_Translate('gettext', KT_ROOT.'language/'.$locale.'.mo', $locale);

		// Deprecated - some custom modules use this to add translations
		Zend_Registry::set('Zend_Translate', KT_I18N::$translation_adapter);

		// Load any local user translations
		if (is_dir(KT_DATA_DIR.'language')) {
			if (file_exists(KT_DATA_DIR.'language/'.$locale.'.mo')) {
				KT_I18N::addTranslation(
					new Zend_Translate('gettext', KT_DATA_DIR.'language/'.$locale.'.mo', $locale)
				);
			}
			if (file_exists(KT_DATA_DIR.'language/'.$locale.'.php')) {
				KT_I18N::addTranslation(
					new Zend_Translate('array', KT_DATA_DIR.'language/'.$locale.'.php', $locale)
				);
			}
			if (file_exists(KT_DATA_DIR.'language/'.$locale.'.csv')) {
				KT_I18N::addTranslation(
					new Zend_Translate('csv', KT_DATA_DIR.'language/'.$locale.'.csv', $locale)
				);
			}
		}

		// Load local user translations from database
		if (KT_DB::isConnected()) {
			$result = KT_DB::prepare("SHOW TABLES LIKE '##custom_lang' ")->execute()->fetchAll(PDO::FETCH_ASSOC);
			if ($result) {
				$translations = KT_DB::prepare(
		...
		self::$list_separator = KT_I18N::translate(', ');

		// I18N: This is the name of the MySQL collation that applies to your language.  A list is available at http://dev.mysql.com/doc/refman/5.0/en/charset-unicode-sets.html
		self::$collation = KT_I18N::translate('utf8_unicode_ci');
	...
	public static function addTranslation(Zend_Translate $translation) {
		KT_I18N::$translation_adapter->addTranslation($translation);
	}

	// Check which languages are installed
	public static function installed_languages() {
		$mo_files = glob(KT_ROOT . 'language' . DIRECTORY_SEPARATOR . '*.mo');
		...
		if (!($installed_languages=self::$cache->load($cache_key))) {
			...
			foreach ($mo_files as $mo_file) {
				if (preg_match('/^(([a-z][a-z][a-z]?)([-_][A-Z][A-Z])?([-_][A-Za-z]+)*)\.mo$/', basename($mo_file), $match)) {
					...
					$tmp1 = KT_I18N::languageName($match[1]);
					if ($match[1] == $match[2]) {
						$tmp2 = $tmp1;
					} else {
						$tmp2 = KT_I18N::languageName($match[2]);
					}
	...
	public static function used_languages() {
		...
		foreach (self::installed_languages() as $code => $name) {
			...
			} elseif (in_array($code, explode(',', KT_Site::preference('LANGUAGES')))) {
				$used_languages[$code] = $name;
			}
		}
	...
	public static function html_markup() {
		...
		return 'class="no-js" lang="' . $lang . '" dir="' . $dir . '"';
	...
	public static function number($n, $precision=0) {
		...
		$n = Zend_Locale_Format::toNumber($n, array('locale'=>KT_LOCALE, 'precision'=>$precision));
		$n = self::digits($n);
	...
	public static function percentage($n, $precision=0) {
		return
			...
			KT_I18N::translate('%s%%', self::number($n*100.0, $precision));
	}

	// echo KT_I18N::translate('Hello World!');
	// echo KT_I18N::translate('The %s sat on the mat', 'cat');
	...
	// echo KT_I18N::translate_c('NOMINATIVE', 'January');
	// echo KT_I18N::translate_c('GENITIVE',   'January');
	...
	public static function gedcom_age($string) {
		...
		if ($age) {
			if (!substr_compare($string, '<', 0, 1)) {
				...
				return KT_I18N::translate('(aged less than %s)', $age);
			} elseif (!substr_compare($string, '>', 0, 1)) {
				// I18N: Description of an individual’s age at an event.  e.g. Died 14 Jan 1900 (aged more than 21 years)
				return KT_I18N::translate('(aged more than %s)', $age);
			} else {
				// I18N: Description of an individual’s age at an event.  e.g. Died 14 Jan 1900 (aged 43 years)
				return KT_I18N::translate('(aged %s)', $age);
			}
		} else {
			// Not a valid string?
			return KT_I18N::translate('(aged %s)', $string);
		}
	}
	...
	public static function datatablesI18N(array $lengths=null) {
		...
		foreach ($lengths as $length) {
			$length_menu.=
				'<option value="'.$length.'">'.
				($length==-1 ? /* I18N: listbox option, e.g. “10,25,50,100,all” */ KT_I18N::translate('All') : self::number($length)).
				'</option>';
		}
		$length_menu='<select>'.$length_menu.'</select>';
		$length_menu=/* I18N: Display %s [records per page], %s is a placeholder for listbox containing numeric options */ KT_I18N::translate('Display %s', $length_menu);
		...
		return
			'"language": {'.
			' "paginate": {'.
			'  "first":    "'./* I18N: button label, first page    */ KT_I18N::translate('first').'",'.
			'  "last":     "'./* I18N: button label, last page     */ KT_I18N::translate('last').'",'.
			'  "next":     "'./* I18N: button label, next page     */ KT_I18N::translate('next').'",'.
			'  "previous": "'./* I18N: button label, previous page */ KT_I18N::translate('previous').'"'.
			' },'.
			' "emptyTable":     "'.KT_I18N::translate('No records to display').'",'.
			' "info":           "'./* I18N: %s are placeholders for numbers */ KT_I18N::translate('Showing %1$s to %2$s of %3$s', '_START_', '_END_', '_TOTAL_').'",'.
			' "infoEmpty":      "'.KT_I18N::translate('Showing %1$s to %2$s of %3$s', 0, 0, 0).'",'.
			' "infoFiltered":   "'./* I18N: %s is a placeholder for a number */ KT_I18N::translate('(filtered from %s total entries)', '_MAX_').'",'.
			' "infoPostfix":    "",'.
