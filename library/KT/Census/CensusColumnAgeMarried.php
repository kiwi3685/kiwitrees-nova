 ...
 */
class KT_Census_CensusColumnAgeMarried extends KT_Census_AbstractCensusColumn implements KT_Census_CensusColumnInterface {
	/**
			...
			foreach ($individual->getSpouseFamilies() as $family) {
				foreach ($family->getFacts('MARR', true) as $fact) {
					if ($fact->getDate()->isOK()) {
						return KT_Date::getAge($individual->getBirthDate(), $fact->getDate(), 0);
					}
				}
			}
		}
