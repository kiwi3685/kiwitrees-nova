 ...
 */
class KT_Census_CensusColumnChildrenDied extends KT_Census_AbstractCensusColumn implements KT_Census_CensusColumnInterface {
	/**
		...
		foreach ($individual->getSpouseFamilies() as $family) {
			foreach ($family->getChildren() as $child) {
				if (
					...
					KT_Date::Compare($child->getDeathDate(), $this->date()) < 0
				) {
					$count++;
				}
			}
