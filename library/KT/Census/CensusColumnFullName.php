 ...
 */
class KT_Census_CensusColumnFullName extends KT_Census_AbstractCensusColumn implements KT_Census_CensusColumnInterface {
	/**
	 ...
	 * @return string[]
	 */
	protected function nameAtCensusDate(KT_Person $individual, KT_Date $census_date) {
		$names = $individual->getAllNames();
		$name  = $names[0];

		foreach ($individual->getSpouseFamilies() as $family) {
			foreach ($family->getFacts('MARR') as $marriage) {
				if ($marriage->getDate()->isOK() && KT_Date::Compare($marriage->getDate(), $census_date) < 0) {
