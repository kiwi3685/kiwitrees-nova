...
class KT_Media extends KT_GedcomRecord
{
	...
	public function getServerFilename($which = 'main')
	{
		...
		if ('main' == $which) {
			// Main image
			return KT_DATA_DIR . $MEDIA_DIRECTORY . $this->file;
		}
		// Thumbnail
		$file = KT_DATA_DIR . $MEDIA_DIRECTORY . 'thumbs/' . $this->file;
		...
		if ($imgsize && ($imgsize[0] && $imgsize[1])) {
			...
			if ($imgsize[0] < $THUMBNAIL_WIDTH) {
				...
				if (hasMemoryForImage($main_file)) {
					switch ($imgsize['mime']) {
						case 'image/png':  $main_image = @imagecreatefrompng($main_file);

						break;

						case 'image/gif':  $main_image = @imagecreatefromgif($main_file);

						break;

						case 'image/jpeg': $main_image = @imagecreatefromjpeg($main_file);
					...
					if ($main_image) {
						...
						switch ($imgsize['mime']) {
							case 'image/png':  @imagepng($thumb_image, $file);

							break;

							case 'image/gif':  @imagegif($thumb_image, $file);

							break;

							case 'image/jpeg': @imagejpeg($thumb_image, $file);
	...
	public function getFilesize($which = 'main')
	{
		...
		return /* I18N: size of file in KB */ KT_I18N::translate('%s KB', KT_I18N::number($size));
	}
	...
	public function getEtag($which = 'main')
	{
		...
		$etag_string = basename($this->getServerFilename($which)) . $this->getFiletime($which) . KT_GEDCOM . KT_USER_ACCESS_LEVEL . $SHOW_NO_WATERMARK;
	 ...
	 * @param mixed $which
	 * @param mixed $addWidth
	 * @param mixed $addHeight
	 *
	 * @return array
	 */
	public function getImageAttributes($which = 'main', $addWidth = 0, $addHeight = 0)
	{
		...
		if ($this->fileExists($which)) {
			...
			if (is_array($imgsize) && !empty($imgsize['0'])) {
				...
				$imgsize['WxH'] = /* I18N: image dimensions, width × height */ KT_I18N::translate('%1$s × %2$s pixels', KT_I18N::number($imgsize['0']), KT_I18N::number($imgsize['1']));
				$imgsize['imgWH'] = ' width="' . $imgsize['adjW'] . '" height="' . $imgsize['adjH'] . '" ';
				if (('thumb' == $which) && ($imgsize['0'] > $THUMBNAIL_WIDTH)) {
	...
	protected function _canDisplayDetailsByType($access_level)
	{
		...
		)->execute([$this->xref, $this->ged_id])->fetchOneColumn();
	...
	protected static function fetchGedcomRecord($xref, $ged_id)
	{
		...
		return $statement->execute([$xref, $ged_id])->fetchOneRow(PDO::FETCH_ASSOC);
	}
}
