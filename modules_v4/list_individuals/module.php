...
class list_individuals_KT_Module extends KT_Module implements KT_Module_List {
	...
	public function getListMenus() {
		...
		$row = KT_DB::prepare(
			"SELECT SQL_CACHE EXISTS(SELECT 1 FROM `##individuals` WHERE i_file=?)"
	...
	public function show() {
		...
		if ($show_all === 'yes') {
			...
			$url = $module_url . 'surname=' . rawurlencode($surname).'&amp;ged=' . KT_GEDURL;
			switch($falpha) {
			case '':
				break;
			case '@':
			...
			$show_all	= 'no';
			$legend		= $UNKNOWN_NN;
			$url		= $module_url . 'alpha=' . rawurlencode($alpha) . '&amp;ged=' . KT_GEDURL;
			$show		= 'indi'; // SURN list makes no sense here
		} elseif ($alpha == ',') {
			$show_all	= 'no';
		...
		<div id="indilist-page" class="grid-x grid-padding-x">
			<div class="cell large-10 large-offset-1">
				...
				foreach (KT_Query_Name::surnameAlpha($show_marnm, false, KT_GED_ID) as $letter=>$count) {
					...
					if ($count) {
						if ($letter == $alpha) {
							$list[] = '<a href="' . $module_url . 'alpha=' . rawurlencode($letter) . '&amp;ged=' . KT_GEDURL . '" class="warning" title="' . $count . '">' . $html . '</a>';
						} else {
							$list[] = '<a href="' . $module_url . 'alpha=' . rawurlencode($letter) . '&amp;ged=' . KT_GEDURL . '" title="' . $count . '">' . $html . '</a>';
						}
					} else {
						$list[] = $html;
					}
				...
				if ($show == 'indi' || $show == 'surn') {
					...
					if ($show == 'surn') {
						...
						if ($show == 'indi') {
							echo format_indi_table(KT_Query_Name::individuals($surname, $alpha, $falpha, $show_marnm, false, KT_GED_ID));
						}
					}
				} ?>
			</div>
