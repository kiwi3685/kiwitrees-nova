...
class widget_review_changes_KT_Module extends KT_Module implements KT_Module_Widget {
			...
			if (KT_USER_CAN_EDIT) {
				...
				foreach ($changes as $change) {
					...
					if ($record->canDisplayDetails()) {
						...
						case 'SOUR':
						case 'OBJE':
							$content.= '<p><a href="'.$record->getHtmlUrl().'">'.KT_I18N::translate('View the changes').'</a></p>';
							break;
						}
						$content.='<br>';
					}
	...
	public function configureBlock($widget_id) {
		if (KT_Filter::postBool('save') && KT_Filter::checkCsrf()) {
			set_block_setting($widget_id, 'days',     KT_Filter::postInteger('num', 1, 180, 7));
			set_block_setting($widget_id, 'sendmail', KT_Filter::postBool('sendmail'));
			exit;
		}
