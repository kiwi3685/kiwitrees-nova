...
class extra_menus_KT_Module extends KT_Module implements KT_Module_Menu, KT_Module_Block, KT_Module_Config {
	...
	private function edit() {
		if (KT_USER_IS_ADMIN) {
			...
			if (KT_Filter::postBool('save') && KT_Filter::checkCsrf()) {
				...
				set_block_setting($block_id, 'menu_title',		KT_Filter::post('menu_title',			KT_REGEX_UNSAFE));
				set_block_setting($block_id, 'menu_address',	KT_Filter::post('menu_address',		KT_REGEX_UNSAFE));
				set_block_setting($block_id, 'menu_access',		KT_Filter::post('menu_access',		KT_REGEX_UNSAFE));
				set_block_setting($block_id, 'new_tab',			KT_Filter::post('new_tab',			KT_REGEX_UNSAFE));
				$languages = array();
	...
	private function config() {
		...
		if ($action == 'update') {
			set_module_setting($this->getName(), 'MENU_TITLE', KT_Filter::post('NEW_MENU_TITLE'));
		...
		<div id="<?php echo $this->getName();?>">
<!--		<a class="current faq_link" href="<?php echo KT_KIWITREES_URL; ?>/faqs/modules-faqs/pages/" target="_blank" rel="noopener noreferrer" title="'. KT_I18N::translate('View FAQ for this page.'). '">'. KT_I18N::translate('View FAQ for this page.'). '<i class="' . $iconStyle . ' fa-comments"></i></a> -->
