...
function privacyStatus($moduleName) {
	...
	if ($death_dates=$controller->record->getAllDeathDates()) {
		...
		if (!$death_date && $SHOW_EST_LIST_DATES) {
			$html .= '<dt>' . KT_I18N::translate('Presumed dead') . help_link('privacy_status', $moduleName) . '</dt>';
			...
			$html .= '<dt>' . KT_I18N::translate('Presumed dead') . help_link('privacy_status', $moduleName) . '</dt>';
			$html .= '<dd>' . isDeadDetail() . '</dd>';
		} else {
			$html .= '<dt>' . KT_I18N::translate('Living') . help_link('privacy_status', $moduleName) . '</dt>';
			$html .= '<dd>' . isDeadDetail() . '</dd>';
		}
