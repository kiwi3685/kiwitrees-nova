...
function print_fact(KT_Event $fact, KT_GedcomRecord $record)
{
	...
	} elseif (preg_match('/2 _WTS @('.KT_REGEX_XREF.')@/', $fact->getGedcomRecord(), $match)) {
		// Event of close relative
		$label_person = KT_Person::getInstance($match[1]);
	} elseif ($fact->getParentObject() instanceof KT_Family) {
		// Family event
	...
	switch ($fact->getTag()) {
			...
			<div class="cell medium-9 fact-detail">
				...
				<?php // Print the spouse and family of this fact/event
				if ($fact->getSpouse()) {
	...
	// Print the place of this fact/event?>
				...
				<?php // Print the associates of this fact/event
				print_asso_rela_record($fact, $record); ?>

				<?php // Print any other "2 XXXX" attributes, in the order in which they appear.
	...
	foreach ($matches as $match) {
		switch ($match[1]) {
						...
						case 'EVEN': // 0 SOUR / 1 DATA / 2 EVEN / 3 DATE / 3 PLAC
							...
							if (count($events) == 1) {
								echo KT_Gedcom_Tag::getLabelValue('EVEN', $event);
							} else {
								echo KT_Gedcom_Tag::getLabelValue('EVEN', implode(KT_I18N::$list_separator, $events));
							}
...
function print_timeline(KT_Event $fact, KT_GedcomRecord $record)
{
	global $HIDE_GEDCOM_ERRORS, $SHOW_FACT_ICONS, $iconStyle ;
	static $n_chil = 0, $n_gchi = 0;
	...
	<div class="cell indiFact <?php echo $styleadd; ?>">
		<div class="grid-x grid-padding-x">
			...
			<?php // Event name -->?>
			<div class="cell small-12 medium-3 small-order-1 medium-order-2 event">
				<?php if ($SHOW_FACT_ICONS) {
					echo $fact->Icon() . '&nbsp;';
				} ?>
				<span class="h6"><?php echo $label; ?></span>
				<?php echo format_fact_date($fact, $record, true, true); ?>
			</div>
			<?php // Place?>
			...
			<?php // Details?>
			<div class="cell <?php echo(KT_USER_CAN_EDIT ? 'small-12 medium-4' : 'auto'); ?> small-order-4 medium-order-4 detail">
			...
			<?php if (KT_USER_CAN_EDIT) { ?>
				<div class="cell small-12 medium-1 small-order-2 medium-order-5 edit">
					<?php if (KT_USER_CAN_EDIT && $styleadd != 'change_old' && $fact->getLineNumber() > 0 && $fact->canEdit()) { ?>
						<div class="editfacts button-group KT-stacked">
							<a class="button clear" onclick="return edit_record('<?php echo $pid; ?>', <?php echo $fact->getLineNumber(); ?>);">
								<i class="<?php echo $iconStyle; ?> fa-pen-to-square"></i>
...
function print_attributes_label(KT_Event $fact, KT_GedcomRecord $record) {
	global $HIDE_GEDCOM_ERRORS, $SHOW_FACT_ICONS, $iconStyle ;
...
function print_attributes_detail(KT_Event $fact, KT_GedcomRecord $record) {
	global $HIDE_GEDCOM_ERRORS, $SHOW_FACT_ICONS, $iconStyle ;
	...
	// -- find source for each fact
	echo print_fact_sources($fact->getGedcomRecord(), 2);
	// -- find notes for each fact
	echo print_fact_notes($fact->getGedcomRecord(), 2);
	//-- find media objects
	echo print_media_links($fact->getGedcomRecord(), 2, $pid);

}

function print_attributes_edit(KT_Event $fact, KT_GedcomRecord $record, $styleadd = '', $type = '') {
	global $HIDE_GEDCOM_ERRORS, $SHOW_FACT_ICONS, $iconStyle ;
	...
	if (KT_USER_CAN_EDIT) {
		if (KT_USER_CAN_EDIT && $styleadd != 'change_old' && $fact->getLineNumber() > 0 && $fact->canEdit()) { ?>
			<div class="editfacts button-group">
				<a class="button clear" onclick="return edit_record('<?php echo $pid; ?>', <?php echo $fact->getLineNumber(); ?>);">
					<i class="<?php echo $iconStyle; ?> fa-pen-to-square"></i>
					<span class="link_text show-for-large" tabindex="1" title="<?php echo KT_I18N::translate('Edit'); ?>" <?php echo $format; ?>>
						<?php echo KT_I18N::translate('Edit'); ?>
					</span>
				</a>
				<a class="button clear" onclick="jQuery.post('action.php',{action:'copy-fact', type:'<?php echo $fact->getParentObject()->getType(); ?>',factgedcom:'<?php echo rawurlencode((string) $fact->getGedcomRecord()); ?>'},function(){location.reload();})">
					<i class="<?php echo $iconStyle; ?> fa-copy"></i>
					<span class="link_text show-for-large" tabindex="2" title="<?php echo KT_I18N::translate('Copy'); ?>" <?php echo $format; ?>>
						<?php echo KT_I18N::translate('Copy'); ?>
					</span>
				</a>
				<a class="button clear" onclick="return delete_fact('<?php echo $pid; ?>', <?php echo $fact->getLineNumber(); ?>, '', '<?php echo KT_I18N::translate('Are you sure you want to delete this fact?'); ?>');">
					<i class="<?php echo $iconStyle; ?> fa-trash-can"></i>
					<span class="link_text show-for-large" tabindex="3" title="<?php echo KT_I18N::translate('Delete'); ?>" <?php echo $format; ?>>
						<?php echo KT_I18N::translate('Delete'); ?>
					</span>
				</a>
			</div>
...
//							onclick="return expand_layer(\'' . $elementID . '\');"
				...
				if (!empty($text)) {
					$data .= '<span>' . KT_Gedcom_Tag::getLabel('PUBL') . ': </span>';
	...
	if (preg_match("/$level ADDR (.*)/", $factrec, $omatch)) {
		...
		if ($level > 1) {
			switch ($format) {
				...
				case 'simple':
					$resultText = str_replace("<br>", "", $resultText);
					$resultText = '<span class="address">'.KT_Gedcom_Tag::getLabel('ADDR').': </span><span>' . $resultText . '</span>';
				break;
				default:
				$resultText = '<span class="address">'.KT_Gedcom_Tag::getLabel('ADDR').': </span><br><div class="indent">' . $resultText . '</div>';
				break;
			}
		}
	...
	for ($j = 0; $j < $ct; $j ++) {
		...
		if (!$source || $source->canDisplayDetails()) {
			...
			if ($cs>0) {
				...
				if ($cs>0) {
					echo '<br>&nbsp;&nbsp;&nbsp;&nbsp;<span class="label">', KT_Gedcom_Tag::getLabel('ROLE'), ' </span><span class="field">', $cmatch[1], '</span>';
				}
			}
...
function printSourceStructure($textSOUR)
{
	...
	if ($textSOUR['PAGE']) {
		$html.='<div class="indent"><span class="">'.KT_Gedcom_Tag::getLabel('PAGE').':</span> <span class="field" dir="auto">'.expand_urls($textSOUR['PAGE']).'</span></div>';
	}

	if ($textSOUR['EVEN']) {
		$html.='<div class="indent"><span class="">'.KT_Gedcom_Tag::getLabel('EVEN').': </span><span class="field" dir="auto">'.$textSOUR['EVEN'].'</span></div>';
		if ($textSOUR['ROLE']) {
			$html.='<div class="indent"><span class="">'.KT_Gedcom_Tag::getLabel('ROLE').': </span><span class="field" dir="auto">'.$textSOUR['ROLE'].'</span></div>';
		}
	}
	...
	if ($textSOUR['DATE'] || count($textSOUR['TEXT'])) {
		if ($textSOUR['DATE']) {
			...
			$html.='<div class="indent"><span class="">'.KT_Gedcom_Tag::getLabel('DATA:DATE').':</span> <span class="field">'.$date->Display(false).'</span></div>';
		}
		foreach ($textSOUR['TEXT'] as $text) {
			$html.='<div class="indent"><span class="">'.KT_Gedcom_Tag::getLabel('TEXT').':</span> <span class="field" dir="auto">'.expand_urls($text).'</span></div>';
		}
	}

	if ($textSOUR['QUAY']!='') {
		$html.='<div class="indent"><span class="">'.KT_Gedcom_Tag::getLabel('QUAY').':</span> <span class="field" dir="auto">'.KT_Gedcom_Code_Quay::getValue($textSOUR['QUAY']).'</span></div>';
	}
...
{
	global $GEDCOM, $SHOW_FACT_ICONS, $TEXT_DIRECTION, $iconStyle;
