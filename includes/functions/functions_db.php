...
function fetch_linked_indi($xref, $link, $ged_id)
{
	...
	)->execute([$ged_id, $link, $xref])->fetchAll(PDO::FETCH_ASSOC);
...
function find_family_record($xref, $ged_id)
{
	...
	return $statement->execute([$xref, $ged_id])->fetchOne();
}
...
function find_updated_record($xref, $ged_id)
{
	...
	// This will return NULL if no record exists, or an empty string if the record has been deleted.
	return $gedcom = $statement->execute([$ged_id, $xref])->fetchOne();
}
...
function gedcom_record_type($xref, $ged_id)
{
	...
	return $statement->execute([$xref, $ged_id, $xref, $ged_id, $xref, $ged_id, $xref, $ged_id, $xref, $ged_id])->fetchOne();
}
...
function get_source_list($ged_id)
{
	...
	usort($list, ['KT_GedcomRecord', 'Compare']);
	...
	if (0 == $max) {
		return
			KT_DB::prepare('SELECT n_surn, COUNT(n_surn) FROM `##name` WHERE n_file=? AND n_type!=? AND n_surn NOT IN (?, ?, ?, ?) GROUP BY n_surn HAVING COUNT(n_surn)>=? ORDER BY 2 DESC')
				->execute([$ged_id, '_MARNM', '@N.N.', '', '?', 'UNKNOWN', $min])
				->fetchAssoc()
			;
	}
...
function get_anniversary_events($jd, $facts = '', $ged_id = KT_GED_ID)
{
	...
	$found_facts = [];
	foreach ([new KT_Date_Gregorian($jd), new KT_Date_Julian($jd), new KT_Date_French($jd), new KT_Date_Jewish($jd), new KT_Date_Hijri($jd), new KT_Date_Jalali($jd)] as $anniv) {
		...
		if (!$anniv instanceof KT_Date_Jewish || in_array($anniv->m, [1, 5, 9, 10, 11, 12, 13])) {
			...
			switch ($anniv->m) {
				case 2:
					...
					} elseif (29 == $anniv->d && 29 == $anniv->DaysInMonth()) {
				...
				case 3:
					...
					if (1 === $anniv->d) {
						$tmp = new KT_Date_Jewish([$anniv->y, 'csh', 1]);
						if (29 == $tmp->DaysInMonth()) {
		...
		foreach ([$ind_sql, $fam_sql] as $sql) {
			...
			foreach ($rows as $row) {
				...
				if ($record->canDisplayDetails()) {
					...
					$ged_date_regex = '/2 DATE.*('.($row['d_day'] > 0 ? "0?{$row['d_day']}\\s*" : '').$row['d_month'].'\\s*'.(0 != $row['d_year'] ? $year_regex : '').')/i';
						...
						if (preg_match('/^1 '.$row['d_fact'].'[ \n]/', $factrec) && preg_match($ged_date_regex, $factrec, $match)) {
							...
							if (canDisplayFact($row['xref'], $ged_id, $factrec)) {
								$found_facts[] = [
									...
									'factrec' => $factrec,
									'jd' => $jd,
									'anniv' => (0 == $row['d_year'] ? 0 : $anniv->y - $row['d_year']),
									'date' => $date,
									'plac' => $plac,
								];
							}
...
function get_calendar_events($jd1, $jd2, $facts = '', $ged_id = KT_GED_ID)
{
	...
	foreach ([$ind_sql, $fam_sql] as $sql) {
		...
		foreach ($rows as $row) {
			...
			0 ? "0?{$row[4]}\\s*" : '').$row[5].'\\s*'.(0 != $row[6] ? $year_regex : '').')/i';
			preg_match_all('/\n(1 ('.KT_REGEX_TAG.').*(\n[2-9] .*)*)/', $row[1], $matches);
			foreach ($matches[1] as $factrec) {
...
function get_gedcom_from_id($ged_id)
{
	...
	if (defined('KT_GED_ID') && defined('KT_GEDCOM') && KT_GED_ID == $ged_id) {
		return KT_GEDCOM;
	}
...
{
	// No need to look up the default gedcom
	if (defined('KT_GED_ID') && defined('KT_GEDCOM') && KT_GEDCOM == $ged_name) {
		return KT_GED_ID;
	}
...
function create_user($username, $realname, $email, $password)
{
	...
	try {
		KT_DB::prepare('INSERT INTO `##user` (user_name, real_name, email, password) VALUES (?, ?, ?, ?)')
			->execute([$username, $realname, $email, $password_hash])
		;
	...
	return KT_DB::prepare('SELECT user_id FROM `##user` WHERE user_name=?')
			->execute([$username])->fetchOne();
}

function rename_user($user_id, $new_username)
{
	KT_DB::prepare('UPDATE `##user` SET user_name=?   WHERE user_id  =?')->execute([$new_username, $user_id]);
}
...
function delete_user($user_id)
{
	...
	KT_DB::prepare('UPDATE `##log` SET user_id=NULL   WHERE user_id =?')->execute([$user_id]);

	// Prevent deletion of users with pending changes.
	$exists_pending = KT_DB::prepare("SELECT 1 FROM `##change` WHERE user_id=? AND status='pending'")->execute([$user_id])->fetchOne();
	if (!$exists_pending) {
		KT_DB::prepare("DELETE FROM `##change`WHERE user_id=? AND status='rejected'")->execute([$user_id]);
		KT_DB::prepare('DELETE `##block_setting` FROM `##block_setting` JOIN `##block` USING (block_id) WHERE user_id=?')->execute([$user_id]);
		KT_DB::prepare('DELETE FROM `##block` WHERE user_id=?')->execute([$user_id]);
		KT_DB::prepare('DELETE FROM `##user_gedcom_setting` WHERE user_id=?')->execute([$user_id]);
		KT_DB::prepare("DELETE FROM `##gedcom_setting` WHERE setting_value=? AND setting_name IN ('CONTACT_USER_ID', 'WEBMASTER_USER_ID')")->execute([(string) $user_id]);
		KT_DB::prepare('DELETE FROM `##user_setting` WHERE user_id=?')->execute([$user_id]);
		if (table_exists('##message')) {
			KT_DB::prepare('DELETE FROM `##message` WHERE user_id=?')->execute([$user_id]);
		}
		KT_DB::prepare('DELETE FROM `##user` WHERE user_id=?')->execute([$user_id]);
	} else {
...
{
	return KT_DB::prepare('SELECT user_id FROM `##user` WHERE email=?')->execute([$email])->fetchOne();
}
...
function check_user_password($user_id, $password)
{
	...
	if (crypt($password, $password_hash) == $password_hash) {
		...
		if (version_compare(PHP_VERSION, '5.3') > 0 && '$2y$12$' != substr($password_hash, 0, 7)) {
...
function get_user_setting($user_id, $setting_name, $default_value = null)
{
	...
	$setting_value = $statement->execute([$user_id, $setting_name])->fetchOne();

	return null === $setting_value ? $default_value : $setting_value;
}
...
function get_user_from_gedcom_xref($ged_id, $xref)
{
	return
		...
		)->execute([$ged_id, 'gedcomid', $xref])->fetchOne();
}
...
function get_gedcomid($user_id, $gedcom_id)
{
	return
		...
		)->execute([$user_id, $gedcom_id, 'gedcomid'])->fetchOne();
}
...
function get_block_setting($block_id, $setting_name, $default_value = null)
{
	...
	$setting_value = $statement->execute([$block_id, $setting_name])->fetchOne();
...
function get_module_setting($module_name, $setting_name, $default_value = null)
{
	...
	$setting_value = $statement->execute([$module_name, $setting_name])->fetchOne();
...
function update_favorites($xref_from, $xref_to, $ged_id = KT_GED_ID)
{
	return
		KT_DB::prepare('UPDATE `##favorites` SET xref=? WHERE xref=? AND gedcom_id=?')
			->execute([$xref_to, $xref_from, $ged_id])
			->rowCount()
		;
}
