...
function format_indi_table($datalist, $option = '')
{
	...
	if ($detect->isMobile() ) {
		$dom = 'dom: \'<"top"p' . $buttons . 'f<"clear">irl><"table-scroll"t><"bottom"pl>\'';
		$shadow = '';
	} else {
		$dom = 'dom: \'<"top"p' . $buttons . 'f<"clear">irl>t<"bottom"pl>\'';
		$shadow = 'shadow';
	}
	...
	$controller
		->addInlineJavascript('
			...
			jQuery("table#' . $table_id . '").addClass("' . $shadow . '");

			jQuery("#buttons-' . $table_id . '")
			/* Hide/show parents */
			.on("click", ".btn-toggle-parents", function() {
				jQuery(this).toggleClass("ui-state-active");
				jQuery(".parents", jQuery("#' . $table_id . '").DataTable().rows().nodes()).slideToggle();
			})
	...
	$filterM = '<span class="M">' . KT_Person::sexImage('M', 'medium') . '</span>';
	$filterF = '<span class="F">' . KT_Person::sexImage('F', 'medium') . '</span>';
	$filterU = '<span class="U">' . KT_Person::sexImage('U', 'medium') . '</span>';
					...
					<tr class="first-row-header">
						<th colspan="24">
							<div class="grid-x grid-margin-x">
								<div class="cell small-2 hollow expanded no-gaps align-left button-group">' .
									hintElement("button", "button", "data-filter-column=\"20\" data-filter-value=\"M\"", KT_I18N::translate('Show only males.'), $filterM) .
									hintElement("button", "button", "data-filter-column=\"20\" data-filter-value=\"F\"", KT_I18N::translate('Show only females.'), $filterF) .
									hintElement("button", "button", "data-filter-column=\"20\" data-filter-value=\"U\"", KT_I18N::translate('Show only individuals with gender unknown.'), $filterU) . '
								</div>
								<div class="cell small-2 hollow align-right no-gaps button-group">' .
								...
								<div class="cell small-3 hollow align-right no-gaps button-group">' .
									...
									hintElement("button", "button", "data-filter-column=\"21\" data-filter-value=\"R\"", KT_I18N::translate('Show individuals who died more than 100 years ago.'), KT_Gedcom_Tag::getLabel('BIRT') . '&lt;=100') . '
								</div>
									<div class="cell small-3 hollow align-center no-gaps button-group">' .
									...
									<div class="cell small-2 hollow align-right no-gaps button-group">' .
									hintElement("button", "button", "data-filter-column=\"21\" data-filter-value=\"Y100\"",  KT_I18N::translate('“Roots” are individuals who have no parents recorded in the database. They may also be called “patriarchs”.'), KT_I18N::translate('Roots')) .
				...
				<table class="shadow scroll" id="' . $table_id . '">
					<thead>
						<tr>
							<th colspan="23">
								<div class="grid-x">
									<div class="cell medium-4 hollow no-gaps align-left button-group">' .
									...
									<div class="cell medium-3 hollow no-gaps align-center button-group">' .
									...
									<div class="cell medium-5 hollow no-gaps align-right button-group">' .
					...
					<tbody>';
						...
						$html .= '
							<td class="text-center">' . KT_I18N::number($nchi) . '</td>
							<td>' . $nchi . '</td>
						';
						// -- Last change
						if ($SHOW_LAST_CHANGE) {
...
function format_sour_table($datalist)
{
	...
	$count_individuals = KT_DB::prepare(
	...
	<table class="shadow scroll" id="' . $table_id . '">
		<thead>
			<tr>
				...
				<th>' . KT_I18N::translate('Media objects') . '</th>
				<th>#OBJE</th>
				<th>' . KT_I18N::translate('Sources') . '</th>
				<th>#SOUR</th>
				<th>' . KT_Gedcom_Tag::getLabel('CHAN') . '</th>
				<th>CHAN</th>
				<th class="delete_src" style="' . (KT_USER_GEDCOM_ADMIN ? '' : 'display: none;') . '">
		...
		->addInlineJavascript('
			...
			jQuery("#' . $table_id . '").dataTable({
				...
				columns: [
					...
					/* 2 file		*/ { visible: ' . (KT_USER_CAN_EDIT || KT_USER_CAN_ACCEPT ? 'true' : 'false') . '},
					/* 3 #indi		*/ { dataSort: 4, class: "text-center" },
					/* 4 #INDI		*/ { type: "num", visible: false },
					/* 5 #fam		*/ { dataSort: 6, class: "text-center" },
					/* 6 #FAM		*/ { type: "num", visible: false },
					/* 7 #sour		*/ { dataSort: 8, class: "text-center" },
					/* 8 #SOUR		*/ { type: "num", visible: false },
	...
	$controller
		...
		->addInlineJavascript('
			...
			columns: [
				...
				/*  2 count */ { dataSort:3, class: "text-center" },
...
function format_surname_list($surnames, $style, $totals, $module_url)
{
	...
	foreach ($surnames as $surn => $surns) {
		...
		if ($surn) {
			$url = $module_url . '&amp;surname=' . urlencode($surn) . '&amp;ged=' . rawurlencode((string) $GEDCOM);
		} else {
			$url = $module_url . '&amp;alpha=,&amp;ged=' . rawurlencode((string) $GEDCOM);
		}
		...
		->addInlineJavascript('
			...
			jQuery("#' . $table_id . '").dataTable({
				...
				columns: [
					...
					/* 1-Record */  { dataSort: 5 },
					/* 2-Change */  ' . (KT_USER_ID ? '{ dataSort: 4 }' : '{ visible: false }') . ',
					/* 3-By */      ' . (KT_USER_ID ? 'null' : '{ visible: false }') . ',
			...
			<table class="shadow scroll" id="' . $table_id . '">
				<thead>
					<tr>
						...
						<th>' . KT_Gedcom_Tag::getLabel('CHAN') . '</th>
						<th>' . KT_Gedcom_Tag::getLabel('_KT_USER') . '</th>
						<th>DATE</th>
						<th>SORTNAME</th>
					</tr>
				</thead>
	...
	if (KT_CLIENT_JD == $endjd) {
		...
		if (0 == $output) {
			if (0 == $filter) {
				...
				$summary = '<div class="callout alert">' .
					KT_I18N::translate('No events for living people exist for today.') .
				'</div>';
			}
		}
	} else {
		...
		if (0 == $output) {
			if (0 == $filter) {
				...
				if ($endjd == $startjd) {
					$summary = '<div class="callout alert">' .
						KT_I18N::translate('No events for living people exist for tomorrow.') .
					'</div>';
				} else {
					$summary = '<div class="callout alert">' .
						// I18N: tanslation for %s==1 is unused; it is translated separately as “tomorrow”
