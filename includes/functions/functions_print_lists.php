		...
		<table class="surname-list" style=visibility: hidden;>
			...
			<tbody>';
				foreach ($surnames as $surn => $surns) {
					...
					if ($surn) {
						$url = $script_url . 'surname=' . rawurlencode($surn) . '&amp;ged=' . KT_GEDURL . '&show_all_firstnames=yes';
					} else {
...
function format_surname_list($surnames, $style, $totals, $module_url) {
	...
	foreach ($surnames as $surn => $surns) {
		...
		if ($surn) {
			$url = $module_url . '&amp;surname=' . urlencode($surn) . '&amp;ged=' . rawurlencode($GEDCOM);
		} else {
			$url = $module_url . '&amp;alpha=,&amp;ged=' . rawurlencode($GEDCOM);
		}
