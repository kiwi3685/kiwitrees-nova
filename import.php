...
if ($row->import_offset == $row->import_total) {
	set_gedcom_setting($gedcom_id, 'imported', '1');
	// Finished?  Show the maintenance links, similar to admin_trees_manage.php
	KT_DB::exec("COMMIT");
	$controller->addInlineJavascript(
		'jQuery("#import' . $gedcom_id . '").addClass("hide");' .
		'jQuery("#actions' . $gedcom_id . '").removeClass("hide");'
	);
	return;
}
...
?>
<div
	id="progressbar<?php echo $gedcom_id; ?>"
	...
	aria-valuemax="100"
>
  <span class="progress-meter" style="width: <?php echo $progress * 100; ?>%; min-width: 40px;">
    <p class="progress-meter-text"><?php echo KT_I18N::percentage($progress, 1); ?></p>
  </span>
</div>
<?php
...
for ($end_time = microtime(true) + 1.0; microtime(true) < $end_time; ) {
	...
	if ($first_time) {
		...
		if (substr($data->chunk_data, 0, 6)!='0 HEAD') {
			...
			$controller->addInlineJavascript('jQuery("#actions' . $gedcom_id . '").removeClass("hide");');
			exit;
		}
