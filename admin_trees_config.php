...
switch (KT_Filter::post('action')) {
	case 'update-general':
		...
		if ($gedcom && KT_GEDCOM != $gedcom) {
			try {
				KT_DB::prepare('UPDATE `##gedcom` SET gedcom_name = ? WHERE gedcom_id = ?')->execute([$gedcom, $gedID]);
				KT_DB::prepare("UPDATE `##site_setting` SET setting_value = ? WHERE setting_name='DEFAULT_GEDCOM' AND setting_value = ?")->execute([$gedcom, KT_GEDCOM]);
			} catch (Exception $ex) {
		...
		set_gedcom_setting($gedID, 'CALENDAR_FORMAT', implode('_and_', array_unique([
	...
	case 'update-resn':
		...
		if (($xref || KT_Filter::post('tag_type')) && KT_Filter::post('resn')) {
			if ('' === KT_Filter::post('xref')) {
				...
				)->execute([$gedID, KT_Filter::post('tag_type')]);
			}
			...
			)->execute([$gedID, $xref, KT_Filter::post('tag_type'), KT_Filter::post('resn')]);
		}
	...
	case 'update-layout':
		...
		set_gedcom_setting($gedID, 'SHOW_RELATIVES_EVENTS', KT_Filter::post('NEW_SHOW_RELATIVES_EVENTS') ? implode(',', KT_Filter::post('NEW_SHOW_RELATIVES_EVENTS')) : '');
	...
	case 'update-edit':
		...
		set_gedcom_setting($gedID, 'INDI_FACTS_QUICK', KT_Filter::post('NEW_INDI_FACTS_QUICK') ? implode(',', KT_Filter::post('NEW_INDI_FACTS_QUICK')) : '');
		set_gedcom_setting($gedID, 'INDI_FACTS_UNIQUE', KT_Filter::post('NEW_INDI_FACTS_UNIQUE') ? implode(',', KT_Filter::post('NEW_INDI_FACTS_UNIQUE')) : '');
		set_gedcom_setting($gedID, 'QUICK_REQUIRED_FACTS', KT_Filter::post('NEW_QUICK_REQUIRED_FACTS') ? implode(',', KT_Filter::post('NEW_QUICK_REQUIRED_FACTS')) : '');
		set_gedcom_setting($gedID, 'FAM_FACTS_ADD', KT_Filter::post('NEW_FAM_FACTS_ADD') ? implode(',', KT_Filter::post('NEW_FAM_FACTS_ADD')) : '');
		set_gedcom_setting($gedID, 'FAM_FACTS_QUICK', KT_Filter::post('NEW_FAM_FACTS_QUICK') ? implode(',', KT_Filter::post('NEW_FAM_FACTS_QUICK')) : '');
		set_gedcom_setting($gedID, 'FAM_FACTS_UNIQUE', KT_Filter::post('NEW_FAM_FACTS_UNIQUE') ? implode(',', KT_Filter::post('NEW_FAM_FACTS_UNIQUE')) : '');
		set_gedcom_setting($gedID, 'QUICK_REQUIRED_FAMFACTS', KT_Filter::post('NEW_QUICK_REQUIRED_FAMFACTS') ? implode(',', KT_Filter::post('NEW_QUICK_REQUIRED_FAMFACTS')) : '');
		set_gedcom_setting($gedID, 'SOUR_FACTS_ADD', KT_Filter::post('NEW_SOUR_FACTS_ADD') ? implode(',', KT_Filter::post('NEW_SOUR_FACTS_ADD')) : '');
		set_gedcom_setting($gedID, 'SOUR_FACTS_QUICK', KT_Filter::post('NEW_SOUR_FACTS_QUICK') ? implode(',', KT_Filter::post('NEW_SOUR_FACTS_QUICK')) : '');
		set_gedcom_setting($gedID, 'SOUR_FACTS_UNIQUE', KT_Filter::post('NEW_SOUR_FACTS_UNIQUE') ? implode(',', KT_Filter::post('NEW_SOUR_FACTS_UNIQUE')) : '');
		set_gedcom_setting($gedID, 'REPO_FACTS_ADD', KT_Filter::post('NEW_REPO_FACTS_ADD') ? implode(',', KT_Filter::post('NEW_REPO_FACTS_ADD')) : '');
		set_gedcom_setting($gedID, 'REPO_FACTS_QUICK', KT_Filter::post('NEW_REPO_FACTS_QUICK') ? implode(',', KT_Filter::post('NEW_REPO_FACTS_QUICK')) : '');
		set_gedcom_setting($gedID, 'REPO_FACTS_UNIQUE', KT_Filter::post('NEW_REPO_FACTS_UNIQUE') ? implode(',', KT_Filter::post('NEW_REPO_FACTS_UNIQUE')) : '');
		set_gedcom_setting($gedID, 'ADVANCED_NAME_FACTS', KT_Filter::post('NEW_ADVANCED_NAME_FACTS') ? implode(',', KT_Filter::post('NEW_ADVANCED_NAME_FACTS')) : '');
		set_gedcom_setting($gedID, 'ADVANCED_PLAC_FACTS', KT_Filter::post('NEW_ADVANCED_PLAC_FACTS') ? implode(',', KT_Filter::post('NEW_ADVANCED_PLAC_FACTS')) : '');
		...
		set_gedcom_setting($gedID, 'PREFER_LEVEL2_SOURCES', KT_Filter::post('NEW_PREFER_LEVEL2_SOURCES', null, '1'));
		set_gedcom_setting($gedID, 'AUTOCOMPLETE_PLACES', KT_Filter::post('NEW_AUTOCOMPLETE_PLACES', null, '0'));
...
$controller
	...
	->addExternalJavascript(KT_TOM_SELECT_JS)
	->addInlineJavascript('
		autocomplete();
		...
		<div class="tabs-content" data-tabs-content="tree_config_tabs">
							...
							<div class="grid-x grid-margin-x">
								<div class="cell large-5">
									<select id="NEW_CALENDAR_FORMAT0" name="NEW_CALENDAR_FORMAT0">
										<?php $CALENDAR_FORMATS = explode('_and_', $CALENDAR_FORMAT);
										if (1 == count($CALENDAR_FORMATS)) {
								...
								<div class="cell callout info-help ">
									<?php
										$d1 = new KT_Date('22 SEP 1792'); $d1 = $d1->Display(false, null, []);
										$d2 = new KT_Date('31 DEC 1805'); $d2 = $d2->Display(false, null, []);
										$d3 = new KT_Date('15 OCT 1582'); $d3 = $d3->Display(false, null, []);
					...
					<div class="grid-x grid-margin-x">
						<div data-abide-error class="calloutnalert" style="display: none;">
							<p>
								...
								<?php echo // I18N: A general error message for forms
			...
			<?php
					...
					$tags = array_unique(array_merge(
						explode(',', get_gedcom_setting($gedID, 'INDI_FACTS_ADD')),
						explode(',', get_gedcom_setting($gedID, 'INDI_FACTS_UNIQUE')),
						explode(',', get_gedcom_setting($gedID, 'FAM_FACTS_ADD')),
						explode(',', get_gedcom_setting($gedID, 'FAM_FACTS_UNIQUE')),
						explode(',', get_gedcom_setting($gedID, 'NOTE_FACTS_ADD')),
						explode(',', get_gedcom_setting($gedID, 'NOTE_FACTS_UNIQUE')),
						explode(',', get_gedcom_setting($gedID, 'SOUR_FACTS_ADD')),
						explode(',', get_gedcom_setting($gedID, 'SOUR_FACTS_UNIQUE')),
						explode(',', get_gedcom_setting($gedID, 'REPO_FACTS_ADD')),
						explode(',', get_gedcom_setting($gedID, 'REPO_FACTS_UNIQUE')),
			...
			<div class="tabs-panel" id="restriction">
				<form method="post" name="configform-resn" action="<?php echo KT_SCRIPT_NAME . '#restriction'; ?>" data-abide novalidate>
					...
					<div class="grid-x grid-margin-x">
						...
						<hr class="cell">
						<?php // Existing restrictions table ?>
						<div class="cell">
									...
									<tr>
										...
										<td>
											<div class="text-center">
												<input type="checkbox" name="del_resn[]" class="check" value="<?php echo $row->default_resn_id; ?>" title="<?php echo KT_I18N::translate('Remove'); ?>">
											</div>
										</td>
									</tr>
									<?php } ?>
				...
				<form method="post" name="configform-layout" action="<?php echo KT_SCRIPT_NAME . '#layout'; ?>" data-abide novalidate>
					...
					<div class="grid-x grid-margin-x">
						...
						<ul class="cell accordion" data-accordion data-allow-all-closed="true">
							...
							<li class="accordion-item" data-accordion-item>
								...
								<div class="accordion-content" data-tab-content>
									<div class="grid-x grid-margin-x">
										...
										<div class="cell large-3">
											<label for="tree_maxnames"><?php echo KT_I18N::translate('Maximum number of surnames on individual list'); ?></label>
										</div>
										<div class="cell large-9">
										...
										<div class="cell large-9">
											<?php echo simple_switch(
												...
												true,
												get_gedcom_setting($gedID, 'SHOW_EST_LIST_DATES')
											); ?>
											<div class="cell callout info-help ">
							...
							<li class="accordion-item" data-accordion-item>
								<?php // Individual page settings ?>
								<a href="#" class="accordion-title clear"><?php echo KT_I18N::translate('Individual page settings'); ?></a>
								<div class="accordion-content" data-tab-content>
									<div class="grid-x grid-margin-x">
										<div class="cell large-3">
											<label for="NEW_SHOW_RELATIVES_EVENTS"><?php echo KT_I18N::translate('Show events of close relatives on individual page'); ?></label>
										</div>
										<div class="cell large-9">
											<select id="NEW_SHOW_RELATIVES_EVENTS" placeholder="<?php echo KT_I18N::translate('Click here to edit selection ...'); ?>" multiple class="tom-select" name="NEW_SHOW_RELATIVES_EVENTS[]">
												<?php $currentTags = explode(',', get_gedcom_setting($gedID, 'SHOW_RELATIVES_EVENTS'));
												foreach (KT_Gedcom_Tag::getPicklistFacts('RELA_EVENTS') as $factId => $factName) {
													$selected = in_array($factId, $currentTags) ? ' selected ' : ' ';
											...
											<div class="grid-x grid-margin-x" id="tree_places">
												<?php echo /* I18N: The placeholders are edit controls. Show the [first/last] [1/2/3/4/5] parts of a place name */ KT_I18N::translate(
														...
														'<div class="cell medium-4 large-2">'.
																select_edit_control(
																	'NEW_SHOW_PEDIGREE_PLACES_SUFFIX',
																	[
							...
							<li class="accordion-item" data-accordion-item>
								...
								<div class="accordion-content" data-tab-content>
									<div class="grid-x grid-margin-x">
										...
										<ul class="cell large-9 accordion" data-accordion>
											<li class="accordion-item is-active" data-accordion-item>
												<?php echo multiSelect(
													'',
													KT_I18N::translate('A list of facts, in addition to Birth and Death, that you want to appear in chart boxes such as the Pedigree chart.'),
													'NEW_CHART_BOX_TAGS',
													'CHART_BOX_TAGS',
													$gedID,
													'ALL-INDI'
			...
			<div class="tabs-panel" id="edit">
				<form method="post" name="configform-edit" action="<?php echo KT_SCRIPT_NAME . '#hide'; ?>" data-abide novalidate>
					...
					<div class="grid-x grid-margin-x">
						...
						<ul class="cell accordion" data-accordion data-allow-all-closed="true">
							<h6 class="success"><?php echo KT_I18N::translate('Facts for Individual records'); ?></h6>
							<li class="accordion-item" data-accordion-item>
								<?php echo multiSelect(
									KT_I18N::translate('All individual facts'),
									KT_I18N::translate('This is the list of GEDCOM facts that your users can add to individuals. You can modify this list by removing or adding fact names, even custom ones, as necessary. <span style="color: #ff0000;">Fact names that appear in this list must not also appear in the <b>Unique individual facts</b> list.</span>'),
									'NEW_INDI_FACTS_ADD',
									'INDI_FACTS_ADD',
									$gedID,
									'ALL-INDI'
							...
							<li class="accordion-item" data-accordion-item>
								<?php echo multiSelect(
									KT_I18N::translate('Unique individual facts'),
									KT_I18N::translate('This is the list of GEDCOM facts that your users can only add <u>once</u> to individuals. For example, if BIRT is in this list, users will not be able to add more than one BIRT record to an individual. <span style="color: #ff0000;">Fact names that appear in this list must not also appear in the <b>All individual facts</b> list.</span>'),
									'NEW_INDI_FACTS_UNIQUE',
									'INDI_FACTS_UNIQUE',
									$gedID,
									'ALL-INDI'
								); ?>
							...
							<li class="accordion-item" data-accordion-item>
								<?php echo multiSelect(
									KT_I18N::translate('Facts for new individuals'),
									KT_I18N::translate('This is a list of GEDCOM fact tags that will be shown when adding a new person. For example, if BIRT is in the list, fields for birth date and birth place will be shown on the form.'),
									'NEW_QUICK_REQUIRED_FACTS',
							...
							<li class="accordion-item" data-accordion-item>
								<?php echo multiSelect(
									KT_I18N::translate('Quick individual facts'),
									KT_I18N::translate('This is the short list of GEDCOM individual facts that appears next to the full list that can be added with a single click.'),
									'NEW_INDI_FACTS_QUICK',
									'INDI_FACTS_QUICK',
									$gedID,
									'ALL-INDI'
								); ?>
							</li>
							<h6 class="success"><?php echo KT_I18N::translate('Facts for Family records'); ?></h6>
							<li class="accordion-item" data-accordion-item>
								<?php echo multiSelect(
									KT_I18N::translate('All family facts'),
									KT_I18N::translate('This is the list of GEDCOM facts that your users can add to families. You can modify this list by removing or adding fact names, even custom ones, as necessary. <span style="color: #ff0000;">Fact names that appear in this list must not also appear in the <b>Unique Family Facts</b> list.</span>'),
									'NEW_FAM_FACTS_ADD',
									'FAM_FACTS_ADD',
									$gedID,
									'ALL-FAM'
							...
							<li class="accordion-item" data-accordion-item>
								<?php echo multiSelect(
									KT_I18N::translate('Unique family facts'),
									KT_I18N::translate('This is the list of GEDCOM facts that your users can only add <u>once</u> to families. For example, if MARR is in this list, users will not be able to add more than one MARR record to a family. <span style="color: #ff0000;">Fact names that appear in this list must not also appear in the <i>Family Add Facts</i> list.</span>'),
									'NEW_FAM_FACTS_UNIQUE',
									'FAM_FACTS_UNIQUE',
									$gedID,
									'ALL-FAM'
								); ?>
							...
							<li class="accordion-item" data-accordion-item>
								<?php echo multiSelect(
									KT_I18N::translate('Facts for new families'),
									KT_I18N::translate('This is a list of GEDCOM fact tags that will be shown when adding a new family. For example, if MARR is in the list, then fields for marriage date and marriage place will be shown on the form.'),
									'NEW_QUICK_REQUIRED_FAMFACTS',
							...
							<li class="accordion-item" data-accordion-item>
								<?php echo multiSelect(
									KT_I18N::translate('Quick family facts'),
									KT_I18N::translate('This is the short list of GEDCOM family facts that appears next to the full list and can be added with a single click.'),
									'NEW_FAM_FACTS_QUICK',
									'FAM_FACTS_QUICK',
									$gedID,
									'ALL-FAM'
							...
							<h6 class="success"><?php echo KT_I18N::translate('Facts for Source records'); ?></h6>
							<li class="accordion-item" data-accordion-item>
								<?php echo multiSelect(
									KT_I18N::translate('All source facts'),
									KT_I18N::translate('This is the list of GEDCOM facts that your users can add to sources. You can modify this list by removing or adding fact names, even custom ones, as necessary. <span style="color: #ff0000;">Fact names that appear in this list must not also appear in the <i>Unique Source Facts</i> list.</span>'),
									'NEW_SOUR_FACTS_ADD',
									'SOUR_FACTS_ADD',
									$gedID,
									'SOUR'
							...
							<li class="accordion-item" data-accordion-item>
								<?php echo multiSelect(
									KT_I18N::translate('Unique source facts'),
									KT_I18N::translate('This is the list of GEDCOM facts that your users can only add <u>once</u> to sources. For example, if TITL is in this list, users will not be able to add more than one TITL record to a source. <span style="color: #ff0000;">Fact names that appear in this list must not also appear in the <i>Source Add Facts</i> list.</span>'),
									'NEW_SOUR_FACTS_UNIQUE',
									'SOUR_FACTS_UNIQUE',
									$gedID,
									'SOUR'
								); ?>
							...
							<li class="accordion-item" data-accordion-item>
								<?php echo multiSelect(
									KT_I18N::translate('Quick source facts'),
								KT_I18N::translate('This is the short list of GEDCOM source facts that appear next to the full list and can be added with a single click.'),
									'NEW_SOUR_FACTS_QUICK',
									'SOUR_FACTS_QUICK',
									$gedID,
									'SOUR'
								); ?>
							</li>
							<h6 class="success"><?php echo KT_I18N::translate('Facts for Repository records'); ?></h6>
							<li class="accordion-item" data-accordion-item>
								<?php echo multiSelect(
									KT_I18N::translate('All repository facts'),
									KT_I18N::translate('This is the list of GEDCOM facts that your users can add to repositories. You can modify this list by removing or adding fact names, even custom ones, as necessary. <span style="color: #ff0000;">Fact names that appear in this list must not also appear in the <i>Unique Repository Facts</i> list.</span>'),
									'NEW_REPO_FACTS_ADD',
									'REPO_FACTS_ADD',
									$gedID,
									'REPO'
							...
							<li class="accordion-item" data-accordion-item>
								<?php echo multiSelect(
									KT_I18N::translate('Unique repository facts'),
									KT_I18N::translate('This is the list of GEDCOM facts that your users can only add <u>once</u> to repositories. For example, if NAME is in this list, users will not be able to add more than one NAME record to a repository. <span style="color: #ff0000;">Fact names that appear in this list must not also appear in the <i>Repository Add Facts</i> list.</span>'),
									'NEW_REPO_FACTS_UNIQUE',
									'REPO_FACTS_UNIQUE',
									$gedID,
									'REPO'
								); ?>
							...
							<li class="accordion-item" data-accordion-item>
								<?php echo multiSelect(
									KT_I18N::translate('Quick repository facts'),
									KT_I18N::translate('This is the short list of GEDCOM repository facts that appear next to the full list and can be added with a single click.'),
									'NEW_REPO_FACTS_QUICK',
									'REPO_FACTS_QUICK',
									$gedID,
									'REPO'
								); ?>
							</li>
							<h6 class="success"><?php echo KT_I18N::translate('Advanced fact settings'); ?></h6>
							<li class="accordion-item" data-accordion-item>
								<?php echo multiSelect(
									KT_I18N::translate('Advanced name facts'),
									KT_I18N::translate('This is a comma separated list of GEDCOM fact tags that will be shown on the add/edit name form. If you use non-Latin alphabets such as Hebrew, Greek, Cyrillic or Arabic, you may want to add tags such as _HEB, ROMN, FONE, etc. to allow you to store names in several different alphabets.'),
									'NEW_ADVANCED_NAME_FACTS',
									'ADVANCED_NAME_FACTS',
									$gedID,
									'NAMES'
								); ?>
							...
							<li class="accordion-item" data-accordion-item>
								<?php echo multiSelect(
									KT_I18N::translate('Advanced place name facts'),
									KT_I18N::translate('This is a comma separated list of GEDCOM fact tags that will be shown on the add/edit name form. If you use non-Latin alphabets such as Hebrew, Greek, Cyrillic or Arabic, you may want to add tags such as _HEB, ROMN, FONE, etc. to allow you to store names in several different alphabets.'),
							...
							<h6 class="success"><?php echo KT_I18N::translate('Other settings'); ?></h6>
							<li class="accordion-item" data-accordion-item>
								<a href="#" class="accordion-title"><?php echo KT_I18N::translate('Surname tradition'); ?></a>
								<div class="cell callout info-help">
								...
								<div class="accordion-content" data-tab-content>
									<?php echo verticalRadioSwitch (
										'NEW_SURNAME_TRADITION',
										surnameDescriptions(),
										get_gedcom_setting($gedID, 'SURNAME_TRADITION'),
										'',
										'Yes',
										'No',
										'tiny'
							...
							<li class="accordion-item" data-accordion-item>
								<a href="#" class="accordion-title"><?php echo KT_I18N::translate('Use full source citations'); ?></a>
								<div class="cell callout info-help">
								...
								<div class="accordion-content" data-tab-content>
									<?php echo simple_switch(
										'NEW_FULL_SOURCES',
										true,
										get_gedcom_setting($gedID, 'FULL_SOURCES'),
										'',
										'Yes',
										'No',
										'tiny'
							...
							<li class="accordion-item" data-accordion-item>
								<a href="#" class="accordion-title"><?php echo KT_I18N::translate('Source type'); ?></a>
								<div class="cell callout info-help">
								...
								<div class="accordion-content" data-tab-content>
									<?php echo verticalRadioSwitch (
										'NEW_PREFER_LEVEL2_SOURCES',
										[
											0 => KT_I18N::translate('None'),
											1 => KT_I18N::translate('Facts'),
											2 => KT_I18N::translate('Records')
										],
										get_gedcom_setting($gedID, 'PREFER_LEVEL2_SOURCES'),
										'',
										'Yes',
										'No',
										'tiny'
							...
							<li class="accordion-item" data-accordion-item>
								<a href="#" class="accordion-title"><?php echo KT_I18N::translate('Use GeoNames database for autocomplete on places'); ?></a>
								<div class="cell callout info-help">
								...
								<div class="accordion-content" data-tab-content>
									<?php echo simple_switch(
										'NEW_USE_GEONAMES',
										true,
										get_gedcom_setting($gedID, 'USE_GEONAMES'),
										'',
										'Yes',
										'No',
										'tiny'
							...
							<li class="accordion-item" data-accordion-item>
								<a href="#" class="accordion-title"><?php echo KT_I18N::translate('Options for autocomplete on places'); ?></a>
								<div class="cell callout info-help">
									<?php echo KT_I18N::translate('The “basic” autocomplete simply searches existing place levels for any that contain a word starting with the string of characters you type, updating as you add characters. No other assistance is provided. <br>
									The "advanced" alternative starts the same, but also provides further features, which might not be compatible with every site\'s preferred naming style:
									<ol><li>After typing a comma and a space at any point it will start the search again, but using subsequent levels of your existing places. This allows, for example, a new town to be added to an existing state or county without re-typing all the existing data.</li>
									<li>Place names are capitalised automatically using "word caps", or capitalising each separate word. This enables faster typing by allowing you to type just lowercase characters.</li></ol>
								...
								<div class="accordion-content" data-tab-content>
									<?php echo verticalRadioSwitch (
										'NEW_AUTOCOMPLETE_PLACES',
										[
											0 => KT_I18N::translate('Basic'),
											1 => KT_I18N::translate('Advanced'),
										],
										get_gedcom_setting($gedID, 'AUTOCOMPLETE_PLACES'),
										'',
										'Yes',
										'No',
										'tiny'
							...
							<li class="accordion-item" data-accordion-item>
								...
								<div class="accordion-content" data-tab-content>
									<?php echo simple_switch(
										'NEW_NO_UPDATE_CHAN',
										true,
										get_gedcom_setting($gedID, 'NO_UPDATE_CHAN'),
										'',
										'Yes',
										'No',
										'tiny'
							...
							<div class="cell card large-3 theme_box <?php echo $selectClass; ?>">
								<div class="card-divider">
									...
									<p class="select text-right">
										<input type="radio" id="radio_<?php echo $themedir; ?>" name="NEW_THEME_DIR" value="<?php echo $themedir; ?>" <?php echo $current_themedir == $themedir ? ' checked="checked"' : ''; ?> data-tooltip title="<?php echo $current_themedir == $themedir ? KT_I18N::translate('Current theme') : KT_I18N::translate('Click here to select this theme'); ?>">
									</p>
								</div>
